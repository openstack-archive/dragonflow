...
OVS_DB_SERICE="ovsdb-server"
OVS_VSWITCHD_SERICE="ovs-vswitchd"
OVS_DIR="/usr/local/var/run/openvswitch"
OVS_DB_PID=$OVS_DIR"/"$OVS_DB_SERICE".pid"
OVS_VSWITCH_PID=$OVS_DIR"/"$OVS_VSWITCHD_SERICE".pid"
...
{
  # Stop ovs db
  ovs_service_stop $OVS_DB_SERICE
  # Stop ovs vswitch
  ovs_service_stop $OVS_VSWITCHD_SERICE

  while ovs_service_status $OVS_DB_SERICE; do
    echo "Waiting for the $OVS_DB_SERICE to be stopped..."
    sleep 1
    ovs_service_stop $OVS_DB_SERICE
  done

  while ovs_service_status $OVS_VSWITCHD_SERICE; do
...
function start_ovs {
    ...
    $OVS_DB_SERICE --remote=punix:$OVS_DIR"/db.sock" \
                 --remote=db:Open_vSwitch,Open_vSwitch,manager_options \
                 --pidfile=$OVS_DB_PID --detach -vconsole:off --log-file $OVSDB_REMOTE \
                 conf.db ${EXTRA_DBS}

    echo -n "Waiting for $OVS_DB_SERICE to start ... "
    ...
    if is_service_enabled df-controller ; then
        ...
        sudo $OVS_VSWITCHD_SERICE --pidfile=$OVS_VSWITCH_PID --detach -vconsole:off --log-file
    fi
