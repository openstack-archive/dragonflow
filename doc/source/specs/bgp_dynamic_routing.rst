...
with distributed SNAT. Further work is needed when both of these two features
have been landed.
...
hosts with BGP application can still advertise the routes. This achieves the
high availability of BGP dynamic routing. But it is not recommended to run BGP
application in every host in a huge cluster. This will create lots of BGP
connections to peer BGP router, and increase the load of peer BGP router
dramatically.
...
A new application for BGP will be added. When BGP speaker has BGP peers and
routes to advertise, any change to routes will cause the BGP application to
advertise or withdraw routes. The changes to BGP peers will cause BGP
application to update the BGP peer connection.

The application will use the BGP drivers at [#]_. Currently, the only
