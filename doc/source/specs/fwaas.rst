...
* If the action is reject, the flow is dropped and an ICMP destination-
  unreachable packet is sent back. For now, OpenvSwitch does not support this
  action, we will implement this by a packet-out event with a rate limiter.
...
Implement reject action by OpenFlow entry once OpenvSwitch can support it.
