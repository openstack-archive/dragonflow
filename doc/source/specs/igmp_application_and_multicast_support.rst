...
More formally:
*Multicast* : VM UUID -> Multicast record (Type: *Multicast record*)
*Multicast record*: Multicast group (Type: *IP*), Source filtering method
    (Type: *'INCLUDE' or 'EXCLUDE'*), filter IPs (Type: *List of IP*)
...
per subnet. The key will be the subnet's UUID. The record value will be a JSON
string representing a map from configuration name to its value, with a
*subnet-id* field containing the subnet's UUID.
