...
It is noted that the current distributed lock has a timeout mechanism.
When the timeout happens, we just log it and continue processing.
The better solution is to reset the distributed lock record in the
database and sync the object from Neutron DB to DF DB after the
Neutron DB transaction is successfully committed. So this also needs the
