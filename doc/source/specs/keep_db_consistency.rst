    ...
    When an API is processing:
        ...
        Acquire the distributed lock for the Neutron object.
            Start Neutron DB transaction for updating version.
                ...
                Increase the version of the corresponding Neutron object.
...
2. Create DB schema for distributed lock and versioned objects. (DONE)
3. Implement distributed lock and versioned objects. (DONE)
4. Protect all the API operations by distributed lock. (DONE)

5. Implement synchronization mechanism [3]. (NOT YET)
At least, we need to provide a sync method on a given neutron object.

In the future, the potential work items:
1. The SQL-based lock is not optimal solution. If DF DB provides
atomic operations on a set of read/write operations, we can refactor
the current SQL-based implementation.

2. Remove Neutron DB. As a result, we don't need to bother the consistency
