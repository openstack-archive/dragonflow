...
implicitly to each port, these rules are not part of the security groups
feature.

With out MAC spoofing protection a VM in a network can pretend and answer
...
This table will have MAC validation rules which blocks any traffic
that has different MAC src address then the MAC address configured for the VM.
