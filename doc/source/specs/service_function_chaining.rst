...
The app is also responsible for terminating NSH encapsulation once the packet
completes last element of the service function chain. Then it is passed it on
to the L2 lookup stage. We will use metadata fields in NSH header to store
source port and network id of the original packet.
...
service function chains that support re-classification (see bug [4]_ and
patch [5]_). The graph stitches together pairs of SFCs, dest-to-source, to mark
...
Neutron service function chaining [6]_ document specifies that when multiple
...
Work Items
----------
#. Make sure openvswitch NSH patches [7]_ get merged
...
.. [6] https://wiki.openstack.org/wiki/Neutron/ServiceInsertionAndChaining#Overview
