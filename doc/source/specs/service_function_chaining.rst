...
database and will take advantage of NSH [3] and openvswitch to implement packet
...
forwards all packets with appropriate SPI/SI pairs.
...
balance between them with select type group actions, using bucket weight we can
implement weighed round robin.
...
service function chains that support re-classification (see bug [4] and
patch [5]). The graph stitches together pairs of SFCs, dest-to-source, to mark
