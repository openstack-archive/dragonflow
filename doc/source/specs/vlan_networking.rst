...
Whereas, there're some demands for other network types, such as vlan, flat, etc.
If vm belongs to vlan network, a vlan header(802.1.Q) will be encapsulated within
the packet, and forwarded to the physical network through the host physical nic.
...
Thus can eliminate arp broadcast for known ports.
With unknown outside servers, thus will be treated as common broadcast.
...
If 'dhcp enalbe' option is off, dhcp broadcast is treated as common broadcast.
...
Actions:  resubmit(,EGRESSTABLE), load_reg7=tunnel_key,resubmit(,EGRESSTABLE,)
...
Inbound
^^^^^^^^^^^
With inbound, a flow item will be installed to table 0, which will strip vlan and set metadata
...
flow items as above.
What's more, there's some special scenario if the deleted port is the last port on this host which
