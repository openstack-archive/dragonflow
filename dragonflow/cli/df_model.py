...
class OASPrinter(ModelsPrinter):
    ...
    def _array_field(self, field_type, is_embedded, restrictions):
        return {'items': self._simple_field(field_type, is_embedded, restrictions),
                'type': 'array'}
    ...
    def handle_field(self, field_name, field_type, is_required, is_embedded,
        ...
        if is_single:
            flds[field_name] = self._simple_field(field_type, is_embedded, restrictions)
        else:
            flds[field_name] = self._array_field(field_type, is_embedded, restrictions)
        if is_required:
