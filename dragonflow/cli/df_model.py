...
def stringify_field_type(field):
...
def process_model(df_model):
    ...
    for key, field in df_model.iterate_over_fields():
        if isinstance(field, field_types.ListOfField):
            to_many = True
            field_type, restrictions = stringify_field_type(field.field)
        elif isinstance(field, fields.ListField):
            to_many = True
            types = field.items_types
            # We will only get the last type
            for field_type in types:
                field_type, restrictions = stringify_field_type(field_type)
        else:
            to_many = False
            field_type, restrictions = stringify_field_type(field)
...
def main():
    ...
    for model in model_framework.iter_models_by_dependency_order(False):
        printer.output_model(process_model(model))
