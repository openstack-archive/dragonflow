...
class SkydiveClient(cotyledon.Service):
    ...
    def __init__(self, nb_api, register_service=False, worker_id=None):
        ...
        if register_service:
            df_service.register_service('df-skydive-service', nb_api, self)
...
# TODO(snapiri): consider removing this as we can now always use cotyledon
...
def service_main():
    ...
    service_manager.add(SkydiveClient, workers=1, args=(nb_api, True))
    service_manager.run()
