...
class WSClientDragonflowProtocol(skydive_client.WSClientDebugProtocol):
    ...
    def _output_table_node(self, nodes, edges, instance):
        ...
        if self.has_owner(instance):
            owner_class = mf.get_table_class(instance.device_owner)
            if not owner_class:
                return
...
def main():
    ...
    client = SkydiveClient.create()
    if args.runtime:
        client.schedule_stop(args.runtime)
    client.clear_dragonflow_items()
    client.start()
