...
class SkydiveClient(cotyledon.Service):
    ...
    def __init__(self, nb_api, worker_id=None):
        ...
        if not logged_in:
            # TODO(snapiri) consider raising an exception
            LOG.error('Failed authenticating with SkyDive analyzer at %s',
                      cfg.CONF.df_skydive.analyzer_endpoint)
            return
        self.prepare_event_loop()
        self.websocket_client.connect()

    # This is required as the client does not check if the loop exists
    # but rather uses it immediately. As the client now runs in a thread,
    # The asyncio will not create the loop automatically.
    # See: https://docs.python.org/3/library/asyncio-eventloops.html
    # Under: Event loop policy interface
    # TODO(snapiri): this should be handled in the library
    @staticmethod
