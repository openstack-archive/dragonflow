...
def model_decorator(f):
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            bottle.abort(404, "Model '%s' not found" % (name,))
...
def create(nbapi, model, name):
    ...
    if not json_data_dict:
        bottle.abort(412, "JSON content required")
...
def update(nbapi, model, name):
    ...
    try:
        ...
        bottle.abort(404,
                     "Model instance '%s/%s' not found" % (name, instnace.id))
...
def get(nbapi, model, name, id_):
    ...
    if not instance:
        bottle.abort(404, "Model instance '%s/%s' not found" % (name, id_))
    bottle.response.content_type = 'application/json'
