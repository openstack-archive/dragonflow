...
class KuryrSubnetDriver(base.PodSubnetsDriver):
    ...
    def _df_to_osvif_routes(self, df_route):
    ...
    def _df_subnet_to_osvif_subnet(self, df_subnet):
        subnet = osv_subnet.Subnet(
            cidr=df_subnet.cidr,
            dns=df_subnet.dns_nameservers,
            routes=self._df_to_osvif_routes(df_subnet.host_routes))
    ...
    def get_subnets(self, pod, project_id):
        osvif_subnet = self._df_subnet_to_osvif_subnet(self.df_subnet)
        osvif_network = self._df_lswitch_to_osvif_network(self.df_lswitch)
        osvif_network.subnets.objects.append(osvif_subnet)
        return {self.df_subnet.id: osvif_network}
