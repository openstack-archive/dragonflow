...
class _Mapping(object):
    def __init__(self, bridge1, bridge2,index=None):
    ...
    def gen_link_mapping(self,
        ...
        if bridge1_link_name is None:
            suffix1 =  self._extract_suffix(self._key[1 - self._bridge1_id])
            bridge1_link_name = "%s-patch" % suffix1
        if bridge2_link_name is None:
            suffix2 = self._extract_suffix(self._key[self._bridge1_id]
            bridge2_link_name = "%s-patch" % suffix2
...
class _BridgeMappings(object):
    ...
    def unique_mapping(self, bridge1, bridge2,
                    mapping_func,
                    bridge1_link_name=None,
                    bridge2_link_name=None):
        ...
        if not self._mappings.get(mapping.key()):
            mapping = mapping.gen_link_mapping(
                    bridge1_link_name,
                    bridge2_link_name)
...
def generate_unique_mapping(bridge1, bridge2, mapping_func,
                     bridge1_link_name=None,
                     bridge2_link_name=None):
