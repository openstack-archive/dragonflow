...
class ActivePortDetectionApp(DFlowApp):

    """A application for detecting the active port for allowed address pairs

    When this application are loaded, packets to the address in a allowed
    address pair will be only sent to the VM port which has been detected
    active for this address. Otherwise, those packets will be broadcast to
    all VM ports with this address in the allowed address pair.
    """

    def __init__(self, *args, **kwargs):
        super(ActivePortDetectionApp, self).__init__(*args,
                                                     **kwargs)
    ...
    def update_local_port(self, lport, original_lport):
        ...
        ips_set = set(self._get_ips_in_allowed_address_pairs(lport))
        original_ips_set = set(self._get_ips_in_allowed_address_pairs(
            original_lport))
