...
class AllowedAddressPairsActiveDetector(DFlowApp):
    ...
    def packet_in_handler(self, event):
        ...
        if (((arp_pkt.opcode == arp.ARP_REQUEST) and
            ...
            self._update_active_node_in_db(
    ...
    def _update_active_node_in_db(self, ip, mac, ofport):
        ...
        active_node = api_nb.AllowedAddressPairsActiveNode({
        ...
        key = network_id + ip
        old_active_node = self.db_store.get_active_node(key)
        if old_active_node != active_node:
            LOG.info(_LI("Detected new active node. ip=%(ip)s, "
                         "mac=%(mac)s, lport_id=%(lport_id)s")
                     % {'ip': ip, 'mac': mac,
                        'lport_id': active_node.get_detected_lport_id()})
            self.nb_api.update_active_node(
                id=key,
                topic=active_node.get_topic(),
                network_id=active_node.get_network_id(),
                ip=active_node.get_ip(),
                detected_mac=active_node.get_detected_mac(),
                detected_lport_id=active_node.get_detected_lport_id())

    def _remove_active_node_from_db_by_lport(self, network_id, ip, lport):
        key = network_id + ip
        old_active_node = self.db_store.get_active_node(key)
        if old_active_node and \
                (old_active_node.get_detected_lport_id() == lport.get_id()):
            self.nb_api.delete_active_node(key, lport.get_topic())
    ...
    def _remove_target_ip(self, ip, lport):
        ...
        self._remove_active_node_from_db_by_lport(lport.get_lswitch_id(), ip,
                                                  lport)
