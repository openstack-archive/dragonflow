...
class DHCPApp(df_base_app.DFlowApp):
    ...
    def _create_port_limter(self, lport):
    ...
    def _check_port_limit(self, lport, src_key):

        port_rate_limiter = self._port_rate_limiters.get(lport.id)

        if port_rate_limiter is None:
            port_rate_limiter = self._create_port_limter(lport)
    ...
    def packet_in_handler(self, event):
        ...
        if self._check_port_limit(lport, unique_key):
    ...
    def _create_dhcp_response(self, packet, dhcp_request,
        ...
        dhcp_server_address = self._dhcp_ip_by_subnet[subnet.id]
    ...
    def _remove_dhcp_port_flow(self, lswitch):
        parser = self.parser
        ofproto = self.ofproto
        match = parser.OFPMatch(metadata=lswitch.unique_key)
        self.mod_flow(
    ...
    def _add_dhcp_ips_by_subnet(self, lport):
        # FIXME (leyal): unify sunets and ips in db-model
        ips_iter = iter(lport.ips)
        subnet_iter = iter(lport.subnets)

        while True:
            try:
                ip = next(ips_iter)
                subnet = next(subnet_iter)
                self._dhcp_ip_by_subnet[subnet.id] = ip
    ...
    def _update_dhcp_ips_by_sbunet(self, lport, orig_lport):
    ...
    def _lport_updated(self, lport, orig_lport):
        ...
        if set(lport.ips) != set(orig_lport.ips) or (
                    lport.mac != orig_lport.mac):
            self._update_port_responders(lport, orig_lport)

        if lport.lswitch.id != orig_lport.lswitch.id:
            self._remove_dhcp_port_flow(orig_lport.lswitch)
            self._install_dhcp_port_flow(lport.lswitch)

        self._update_dhcp_ips_by_sbunet(lport, orig_lport)

    def _delete_dhcp_ips_by_subent(self, lport):
    ...
    def _lport_deleted(self, lport):
        ...
        self._remove_dhcp_port_flow(lport.lswitch)
        self._delete_dhcp_ips_by_subent(lport)
    ...
    def _install_dhcp_port_icmp_responder(self, lport):
        for ip in lport.ips:
    ...
    def _remove_dhcp_port_icmp_responder(self, lport):
        for ip in lport.ips:
            icmp_responder.ICMPResponder(
                app=self,
                network_id=lport.lswitch.unique_key,
