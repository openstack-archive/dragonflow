...
class DHCPApp(df_base_app.DFlowApp):
    ...
    def _lport_updated(self, lport, orig_lport):
        ...
        v4_ips = set(ip for ip in lport.ips if ip.version == 4)
        v4_old_ips = set(ip for ip in orig_lport.ips if ip.version == 4)
    ...
    def _install_dhcp_port_icmp_responder(self, lport):
        ips_v4 = (ip for ip in lport.ips if ip.version == 4)
        for ip in ips_v4:
