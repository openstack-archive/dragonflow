...
class DNATApp(df_base_app.DFlowApp):
    ...
    def _remove_ingress_nat_rules(self, floatingip):
        ...
        # Remove translate + ICMP flows
        for priority in (const.PRIORITY_MEDIUM, const.PRIORITY_HIGH):
            self.mod_flow(
                command=self.ofproto.OFPFC_DELETE_STRICT,
                table_id=const.INGRESS_DNAT_TABLE,
                priority=priority,
    ...
    def _update_floatingip(self, floatingip, orig_floatingip):
        ...
        if orig_floatingip.lport is not None:
            ...
            self._uninstall_floatingip(
                orig_floatingip,
                update_status=floatingip.lport is None,
            )

        if floatingip.lport is not None:
            # Update here only if we're associating
            self._install_floatingip(
                floatingip,
                update_status=orig_floatingip.lport is None,
            )
    ...
    def _install_floatingip(self, floatingip, update_status=True):
        if floatingip.lport.is_local:
            self._install_local_floatingip(floatingip, update_status)
        elif floatingip.lport.is_remote:
            self._install_remote_floatingip(floatingip)

    def _uninstall_floatingip(self, floatingip, update_status=True):
        if floatingip.lport.is_local:
            self._uninstall_local_floatingip(floatingip, update_status)
        elif floatingip.lport.is_remote:
            self._uninstall_remote_floatingip(floatingip)

    def _install_local_floatingip(self, floatingip, update_status):
        floatingip.floating_lport.emit_local_created()
    ...
    def _uninstall_local_floatingip(self, floatingip, update_status):
        if update_status:
            self.update_floatingip_status(
        ...
        floatingip.floating_lport.emit_local_deleted()

    def _install_remote_floatingip(self, floatingip):
        floating_lport = floatingip.floating_lport.get_object()
        floating_lport.binding = floatingip.lport.binding
        floating_lport.emit_remote_created()

    def _uninstall_remote_floatingip(self, floatingip):
        floating_lport = floatingip.floating_lport.get_object()
        floating_lport.binding = floatingip.lport.binding
        floating_lport.emit_remote_deleted()
    ...
    def _add_local_port(self, lport):
        for floatingip in self._get_floatingips_by_lport(lport):
            self._install_local_floatingip(floatingip, update_status=True)

    @df_base_app.register_event(l2.LogicalPort, l2.EVENT_LOCAL_UPDATED)
    def _update_local_port(self, lport, orig_lport):
        for floatingip in self._get_floatingips_by_lport(lport):
            self._uninstall_local_floatingip(floatingip, update_status=True)
            self._install_local_floatingip(floatingip, update_status=False)

    @df_base_app.register_event(l2.LogicalPort, l2.EVENT_LOCAL_DELETED)
    def _remove_local_port(self, lport):
        for floatingip in self._get_floatingips_by_lport(lport):
            # FIXME (dimak) status update: what about migration?
            self._uninstall_local_floatingip(floatingip, update_status=True)
    ...
    def _update_remote_port(self, lport, orig_lport):
        if lport.binding == orig_lport.binding:
            return
