...
class DNATApp(df_base_app.DFlowApp):
    ...
    def _uninstall_floatingip(self, floatingip):
        # When the VM was deleted, the nova port is removed. (Actually the
        # floating port was also removed). As the lport is actually a proxy,
        # and the port does not exist now, we should just return in that case
