...
class FcApp(df_base_app.DFlowApp):
    ...
    def _local_lport_added(self, lport):
        for fc in self._flow_classifiers_by_lport(lport):
            ...
            if fc.source_port is not None and fc.source_port.id == lport.id:
                self._install_classification_flows(fc)
            elif fc.dest_port is not None and fc.dest_port.id == lport.id:
                    ...
                    self.parser.OFPInstructionActions(
                        ...
                        [
                            self.parser.OFPActionSetField(
                                reg6=flow_classifier.unique_key),
                        ],
                    ),
    ...
    def _install_dispatch_flows(self, flow_classifier):
        ...
        # End-of-chain
        # 1) Restore network ID in metadata and zero reg6
        ...
        self.mod_flow(
            ...
            match=self.parser.OFPMatch(reg6=flow_classifier.unique_key),
            actions=actions,
        )
