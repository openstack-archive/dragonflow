...
class ChassisSNATApp(df_base_app.DFlowApp, snat_mixin.SNATApp_mixin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # override <None> default in test mode
        # to prevent fulltest crash
        if cfg.CONF.df_snat_app.test_mode is True:
            self.external_host_ip = '172.24.4.100'
    ...
    def switch_features_handler(self, ev):
        ...
        # install strategy flows on first VM port
        if self.external_host_ip is None:
            LOG.warning(_LW("SNAT application is disabled, "
                            "please supply external_host_ip conf. parameter"))
        else:
