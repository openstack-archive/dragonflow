...
class CIDRList(object):
    ...
    def _if_cidrs_can_be_merged(cidr1, cidr2):
        """Internal interface. Check those two cidrs if can be merged.
           @param cidr1 - IPNetwork
           @param cidr2 - IPNetwork
        """
    ...
    def _remove_one_address(cidr_list, address_to_remove):
        """Internal interface. Remove one address from the cidr list, and get
           a new cidr list and the changed cidrs.
           @param cidr_list - IPNetwork list
           @param address_to_remove - IPAddress
        """
        added_cidr = []
        removed_cidr = []
    ...
    def _add_one_address(cidr_list, address_to_add):
        """Internal interface. Add one address to the cidr list, and get a new
           cidr list and the changed cidrs.
           @param cidr_list - IPNetwork list
           @param address_to_add - IPAddress
        """
        position = None
    ...
    @staticmethod
    def _get_new_cidr_list_and_changes(cidr_list, addresses_to_add,
                                       addresses_to_remove):
        """Internal interface. Update addresses in cidr list, and get a new
           cidr list and the changed cidrs.
           @param addresses_to_add - IPAddress or string list
           @param addresses_to_remove - IPAddress or string list
        """
        added_cidr = []
        removed_cidr = []
        new_cidr_list = cidr_list
        ...
        return new_cidr_list, filtered_added_cidr, filtered_removed_cidr
    ...
    def remove_addresses_and_get_changes(self, address_list):
        """Remove addresses from the cidr list, and get the changed cidrs.
           @param address_list - IPAddress or string list
        """
        self._list, added_cidr, removed_cidr = \
            self._get_new_cidr_list_and_changes(self._list, [], address_list)

        return added_cidr, removed_cidr

    def add_addresses_and_get_changes(self, address_list):
        """Add addresses to the cidr list, and get the changed cidrs.
           @param address_list - IPAddress or string list
        """
        self._list, added_cidr, removed_cidr = \
            self._get_new_cidr_list_and_changes(self._list, address_list, [])
    ...
    def update_addresses_and_get_changes(self, addresses_to_add,
                                         addresses_to_remove):
        """Update addresses in cidr list, and get a new cidr list
           and the changed cidrs.
           @param addresses_to_add - IPAddress or string list
           @param addresses_to_remove - IPAddress or string list
        """
        self._list, added_cidr, removed_cidr = \
            self._get_new_cidr_list_and_changes(self._list, addresses_to_add,
                                                addresses_to_remove)
