...
class LogicalNetworks(object):
    ...
    def get_ports(self, ports, network_id, network_type=None):
        network_ports = ports.get(network_id)
        if network_ports:
            if network_type:
    ...
    def add_local_port(self, **kargs):
        return self.add_port(self.local_net, **kargs)

    def add_remote_port(self, **kargs):
        return self.add_port(ports=self.remote_net, **kargs)

    def remove_local_port(self, **kargs):
        return self.remove_port(ports=self.local_net, **kargs)

    def remove_remote_port(self, **kargs):
        return self.remove_port(ports=self.remote_net, **kargs)

    def get_local_port_count(self, **kargs):
        return self.get_port_count(ports=self.local_net, **kargs)

    def get_remote_port_count(self, **kargs):
        return self.get_port_count(ports=self.remote_net, **kargs)

    def get_local_ports(self, **kargs):
        return self.get_ports(ports=self.local_net, **kargs)

    def get_remote_ports(self, **kargs):
        return self.get_ports(ports=self.remote_net, **kargs)
