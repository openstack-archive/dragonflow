...
class LogicalNetworks(object):
    """
    LogicalNetworks is a common infra that holds different types of network
    ports. and allows a multi technolgy multi segmented port caching
    """
    def __init__(self):
        initialiser = functools.partial(collections.defaultdict, list)
        self.local_net = collections.defaultdict(initialiser)
        self.remote_net = collections.defaultdict(initialiser)
