...
class DfLocalController(object):
    ...
    def _set_lport_external_value_by_lswitch(self, lswitch, lport):
        ...
        if lswitch_unique_key is None or not network_type:
    ...
    def _local_lport_process(self, lport, original_lport):
    ...
    def _remote_lport_process(self, lport, original_lport):
    ...
    def _logical_port_process(self, lport, original_lport=None):
        ...
        if chassis == self.chassis_name:
            if self._local_lport_process(lport, original_lport):
                return
        else:
            if self._remote_lport_process(lport, original_lport):
                return
