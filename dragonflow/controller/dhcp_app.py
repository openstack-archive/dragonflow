...
class DHCPApp(df_base_app.DFlowApp):
    ...
    def _create_dhcp_packet(self, packet, dhcp_packet, pkt_type, lport):
        ...
        if not dhcp_server_address:
            LOG.error("Did not find DHCP server address for subnet")
            LOG.error("Subnet: %r", subnet)
            LOG.error("Subnet in db_store2: %r", self.db_store2.get_one(subnet))
    ...
    def update_logical_switch(self, lswitch, orig_lswitch=None):
        print ('update_logical_switch: Enter version: %s' % lswitch.version)
