...
class DHCPApp(df_base_app.DFlowApp):
    ...
    def _create_dhcp_ack(self, pkt, dhcp_packet, lport):
        ...
        bootfile_name = self._get_extra_dhcp_opts_value(lport, 'bootfile-name')
        if not bootfile_name:
            bootfile_name = dhcp_packet.boot_file
        else:
            server_name = self._get_extra_dhcp_opts_value(lport, 'tftp-server')
    ...
    def _create_dhcp_offer(self, pkt, dhcp_packet, lport):
        ...
        if not bootfile_name:
            ...
            server_name = self._get_extra_dhcp_opts_value(lport, 'server-name')
