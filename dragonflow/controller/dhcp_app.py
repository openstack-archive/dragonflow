...
class DHCPApp(df_base_app.DFlowApp):
        ...
        for subnet in subnets:
            if self._is_ipv4(subnet):
                ...
                if subnet.enable_dhcp():
                    ...
                    if (subnet_id not in self.switch_dhcp_ip_map[network_id] or
                        not self.switch_dhcp_ip_map[network_id][subnet_id]):
                        # The first time the subnet is found as a dhcp enabled
                        # subnet.
                        self._install_dhcp_unicast_match_flow(dhcp_ip,
                                                              network_id)
                        self._install_dhcp_flow_for_vm_in_sunbet(subnet_id)
                        self.switch_dhcp_ip_map[network_id].update(
                            {subnet_id: dhcp_ip})
                else:
                    if (subnet_id in self.switch_dhcp_ip_map[network_id] and
                        self.switch_dhcp_ip_map[network_id][subnet_id]):
                        # The subnet was found as a dhcp enabled subnet, but it
                        # has been changed to dhcp disabled subnet now.
                        self._uninstall_dhcp_flow_for_vm_in_sunbet(subnet_id)
    ...
    def _install_dhcp_flow_for_vm_in_sunbet(self, subnet_id):
    ...
    def _uninstall_dhcp_flow_for_vm_in_sunbet(self, subnet_id):
        local_ports = self.subnet_vm_port_map[subnet_id]
        for p_id in local_ports:
            port = self.db_store.get_local_port(p_id)
            if port:
