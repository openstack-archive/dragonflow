...
class DNATApp(DFlowApp):
    ...
    def packet_in_handler(self, event):
        ...
        if arp_pkt.opcode == arp.ARP_REPLY:
            ...
            if fip:
                self.self._update_external_gateway_mac(fip, arp_pkt.src_mac)
    ...
    def _create_external_bridge(self):
        self.external_ofport = self.vswitch_api.create_patch_port('br-int',
                                           self.ex_peer_patch_port,
                                           self.int_peer_patch_port)
        self.vswitch_api.create_patch_port(self.external_network_bridge,
                                           self.int_peer_patch_port,
                                           self.ex_peer_patch_port)

    def _set_external_network(self, network_id):
        refcnt = self.external_networks.get(network_id, None)
        if refcnt is None:
            self.external_networks[network_id] = 1
        else:
            self.external_networks[network_id] += 1

    def _remove_external_network(self, network_id):
        refcnt = self.external_networks.get(network_id, None)
        if refcnt:
            if refcnt == 1:
                del self.external_networks[network_id]
            elif refcnt > 1:
                self.external_networks[network_id] -= 1
    ...
    def _get_arp_reply_match(self, destination_ip, network_id=None):
    ...
    def _get_gratuitous_arp_match(self, destination_ip, network_id=None):
    ...
    def _get_packet_in_instructions(self, floatingip_id):
        parser = self.get_datapath().ofproto_parser
    ...
    def _install_mac_learning_rules(self, floatingip):
        # process arp reply
        match = self._get_arp_reply_match(
            floatingip.ip_address)
        instructions = self._get_packet_in_instructions(floatingip.name)
        self.mod_flow(
            self.get_datapath(),
            inst=instructions,
        ...
        match = self._get_gratuitous_arp_match(
    ...
    def _install_ingress_nat_rules(self, floatingip):
        ...
        if not self.db_store.get_floatingips():
        ...
        self._set_external_network(network_id)
    ...
    def _remove_ingress_nat_rules(self, floatingip):
        ...
        match = parser.OFPMatch()
        match.set_in_port(self.external_ofport)
        local_floatingips = self.db_store.get_floatingips()
        if local_floatingips and (len(local_floatingips) == 1):
        ...
        self._remove_external_network(network_id)
