...
DF_DNAT_APP_OPTS = [
    ...
    cfg.StrOpt('ex_peer_patch_port', default='patch-int',
               help=_("Peer patch port in external bridge for integration "
                      "bridge.")),
    cfg.StrOpt('send_arp_interval', default=5,
...
class DNATApp(DFlowApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.send_arp_interval = int(cfg.CONF.df_dnat_app.send_arp_interval)
    ...
    def _send_arp_request_entry(self):
        ...
        while True:
            eventlet.sleep(self.send_arp_interval)
            for k, v in six.iteritems(self.external_gateway_mac):
                fip = None
                # if v is none, then need to get the gw mac
                if v is None:
                    fip = self.db_store.get_first_floatingip(k)
                if fip:
                    # request gw arp
    ...
    def _periodic_send_arp_request(self):
        ...
        eventlet.spawn(self._send_arp_request_entry)
    ...
    def packet_in_handler(self, event):
        ...
        if (arp_pkt.opcode == arp.ARP_REQUEST and
            ...
            gw_fips = self.db_store.get_floatingip_by_gateway(arp_pkt.src_ip)
            if not gw_fips:
                return
    ...
    def _decrease_external_network_count(self, network_id):
        self.external_networks[network_id] -= 1
        if self.external_networks[network_id] == 0:
            self.external_networks[network_id]
    ...
    def _install_dnat_egress_rules(self, floatingip):
        ...
        gw_mac = self.external_gateway_mac.get(net_id, None)
        if not gw_mac:
            self.external_gateway_mac[net_id] = None
            self._send_gw_arp_request(floatingip)
            floatingip.update_fip_status('resolving')
            return
