...
class L2App(df_base_app.DFlowApp):
    def __init__(self, *args, **kwargs):
        ...
        self.is_install_nd_advertiser = cfg.CONF.df_l2_app.l2_responder
    ...
    def _add_nd_advertiser(self, lport):
        if not self.is_install_nd_advertiser:
            return
        ips = lport.get_ip_list()
        for ip in ips:
            ...
            network_id = lport.get_external_value('local_network_id')
            mac = lport.get_mac()
            nd_advertisers.NsAdvertiser(self,
                                        network_id, ip, mac).add()

    def _remove_nd_advertiser(self, lport):
        if not self.is_install_nd_advertiser:
            return
        ips = lport.get_ip_list()
        for ip in ips:
    ...
    def remove_local_port(self, lport):
        ...
        self._remove_nd_advertiser(lport)
