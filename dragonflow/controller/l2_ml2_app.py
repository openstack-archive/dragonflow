...
class L2App(DFlowApp):
    ...
    def _del_network_flows_for_tunnel(self, segmentation_id):
        LOG.info("Delete network flows for tunnel.")
        if segmentation_id is None:
            return
    ...
    def _add_multicast_broadcast_handling_for_remote_port(self,
                                                          ...
                                                          segmentation_id,
                                                          ofport):
        LOG.info("Adding multicast and broadcast for remote port.")
        parser = self.get_datapath().ofproto_parser
        ofproto = self.get_datapath().ofproto
    ...
    def add_remote_port(self, lport):
        ...
        LOG.info("Add remote with segmentation.")
    ...
    def _install_network_flows_on_first_port_up(self,
        ...
        if network_type == 'vlan' or network_type == 'flat':
            self._install_network_flows_for_vlan_and_flat(network_type,
                                                          segmentation_id,
                                                          local_network_id)
        else:
    ...
    # Install network flows for vlan and flat
    # Table=L2_LOOKUP_TABLE, Proority = PRIORITY_MEDIUM
    # Match: metadata=network_id, dl_dst=00:00:00:00:00:00/01:00:00:00:00:00
    # Actions: goto "EGRESS"
    def _install_network_flows_for_vlan_and_flat(self, network_type,
                                                 segmentation_id,
                                                 local_network_id, ):
        LOG.info(_LI("Install network flows on first %s up")
                 % network_type)
    ...
    def _install_network_flows_for_vlan(self, segmentation_id,
        ...
        # Actions: metadata=network_id, goto 'Destiantion Port Classification'
        match = parser.OFPMatch()
        match.set_vlan_vid(segmentation_id)
        actions = []
        actions.append(parser.OFPActionSetField(metadata=local_network_id))
    ...
    def _install_network_flows_for_flat(self, local_network_id):
        ...
        # Actions: metadta=network_id,  goto 'Destiantion Port Classification'
