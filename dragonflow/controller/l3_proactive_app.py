...
class L3ProactiveApp(DFlowApp):
    ...
    def add_router_route(self, router, route):
        ...
        actions = [
            parser.OFPActionDecNwTtl(), parser.OFPActionSetField(eth_dst=dst_mac),
        ...
        self.mod_flow(
            self.get_datapath(), cookie=tunnel_key, inst=inst,
            table_id=const.L3_LOOKUP_TABLE, priority=const.PRIORITY_VERY_HIGH,
            match=match)
    ...
    def remove_router_route(self, router, route):
        ...
        self.mod_flow(
            self.get_datapath(), command=ofproto.OFPFC_DELETE,
            table_id=const.L3_LOOKUP_TABLE, priority=const.PRIORITY_VERY_HIGH,
