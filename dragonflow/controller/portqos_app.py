...
class PortQosApp(df_base_app.DFlowApp):
    ...
    @df_base_app.register_event(qos.QosPolicy, 'created')
    @df_base_app.register_event(qos.QosPolicy, 'updated')
    def update_qos_policy(self, policy):
        LOG.debug('update_qos_policy: Enter')
    ...
    @df_base_app.register_event(qos.QosPolicy, 'deleted')
    def delete_qos_policy(self, policy):
        LOG.debug('delete_qos_policy: Enter')
        local_ports = self.db_store.get_local_ports(policy.topic)
        for port in local_ports:
            if port.get_qos_policy_id() == policy.id:
