...
class PortQosApp(DFlowApp):
    ...
    def add_local_port(self, lport):
        ...
        ovs_port_id = lport.get_ovs_port_id()
        self.vswitch_api.add_port_qos(ovs_port_id, qos)
        return
    ...
    def update_local_port(self, lport, original_lport):
        ...
        if new_qos_id is None:
            if old_qos_id is None:
                ...
                self.vswitch_api.del_port_qos(ovs_port_id)
        else:
    ...
    def update_qos(self, qos):
        local_ports = self.db_store.get_local_ports()
        for value in local_ports:
            if value['qos_policy_id'] == qos.get_id():
                ovs_port_id = value['ovs_port_id']
                self.vswitch_api.del_port_qos(ovs_port_id)
                self.vswitch_api.add_port_qos(ovs_port_id, qos)
        return
