...
IPV4_MULTICAST_ETHERNET_MAC_ADDRESS = '01:00:5e:00:00:00'
IPV4_MULTICAST_ETHERNET_MAC_MASK = 'ff:ff:ff:80:00:00'
...
class PortSecApp(DFlowApp):
    ...
    def _install_flows_check_valid_mac(self, datapath, ofport, mac):
        ...
        match = parser.OFPMatch(
            in_port=ofport,
            eth_src=mac,
            eth_dst=(IPV4_MULTICAST_ETHERNET_MAC_ADDRESS,
                     IPV4_MULTICAST_ETHERNET_MAC_MASK),
            eth_type=ether.ETH_TYPE_IP
        )
        self.add_flow_go_to_table(datapath,
                                  const.EGRESS_PORT_SECURITY_TABLE,
    ...
    def update_local_port(self, lport, original_lport):
        ...
        if enable:
            if original_enable:
                ...
                self._install_port_security_flows(lport, datapath)
                self._uninstall_disable_flow(original_lport, datapath)
        else:
            if original_enable:
                self._install_disable_flow(lport, datapath)
