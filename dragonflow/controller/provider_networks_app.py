...
class ProviderNetworksApp(df_base_app.DFlowApp):
    ...
    def _setup_physical_bridges(self, bridge_mappings):
        ...
        #TODO(oanson) Detect bridge type. For OVS - use patch ports. For Linux bridge - use veth pairs
    ...
    def add_local_port(self, lport):
        ...
        #TODO(oanson) Add treatment for router:external=External OR add special treatment for floatingip ports OR Add special treatment for (assoc) floating ip
    ...
    def _egress_flow(self, lport, network_id, network_type):
        ...
        if network_type == NET_VLAN:
            segmentation_id = lport.get_external_value('segmentation_id')
            vlan_vid = (segmentation_id & VLAN_MASK)
            actions = [
