...
VLAN_TAG_BITS = 12
VLAN_MASK = utils.set_bitmask(VLAN_TAG_BITS)
...
class ProviderNetworksApp(df_base_app.DFlowApp):
    ...
    def _setup_physical_bridges(self, bridge_mappings):
        ...
        #TODO(oanson) Detect bridge type. For OVS - use patch ports.
        #For Linux bridge - use veth pairs
        #TODO(oanson) Rename patch ports names
    ...
    def add_local_port(self, lport):
        ...
        #TODO(oanson) Add treatment for router:external=External OR add
        #special treatment for floatingip ports OR Add special treatment
        #for (assoc) floating ip
    ...
    def _new_network_flow(self, lport, network_id, network_type):
        LOG.info(_LI('new %(net_type)s network: %(net_id)s'),
                {'net_type': network_type,
                 'net_id': network_id})
    ...
    def _l2_lookup_flow(self, network_id):
        LOG.info(_LI('l2 lookup flow for network %(net_id)s'),
                {'net_id': network_id})
    ...
    def _egress_flow(self, lport, network_id, network_type):
        LOG.info(_LI('egrees flow for network %(net_id)s'),
                {'net_id': network_id})
        ...
        if network_type == NET_VLAN:
            ...
            actions = [
                    ...
                    self.parser.OFPActionSetField(
                        vlan_vid=vlan_tag | self.ofproto.OFPVID_PRESENT)]
    ...
    def _egress_external_flow(self, lport, network_id):
        LOG.info(_LI('egrees external flow for network %(net_id)s'),
                {'net_id': network_id})
    ...
    def _network_classification_flow(self, lport, network_id, network_type):
        LOG.info(_LI('network classification flow for network_id: %(net_id)s'),
                {'net_id': network_id})
