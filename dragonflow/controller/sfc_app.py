...
class SfcApp(df_base_app.DFlowApp):
    ...
    @log_helpers.log_method_call
    @df_base_app.register(models_sfc.PortPairGroup, 'updated')
    ...
    def sfc_portchain_deleted(self, pc):
        ...
        driver.uninstall_encap_flows(pc)
        driver.uninstall_decap_flows(pc)
...
class MplsDriver(object):
    ...
    @log_helpers.log_method_call
    def uninstall_encap_flows(self, pc):
        for fc_idx, fc in enumerate(pc.flow_classifiers):
            # FIXME only relevant fc
            self.app.mod_flow(
    ...
    @log_helpers.log_method_call
    def uninstall_decap_flows(self, pc):
        for fc_idx, fc in enumerate(pc.flow_classifiers):
            last_ppg_idx = len(pc.port_pair_groups) - 1
