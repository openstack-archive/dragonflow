...
class MplsDriver(object):
    ...
    @log_helpers.log_method_call
    def install_forward_to_dest(self, pc, fc):
        for eth_type in self._ETH_TYPE_TO_TC:
            self.app.mod_flow(
                table_id=constants.SFC_MPLS_DISPATCH_TABLE,
                priority=constants.PRIORITY_HIGH,
    ...
    @log_helpers.log_method_call
    def uninstall_forward_to_dest(self, pc, fc):
        for eth_type in self._ETH_TYPE_TO_TC:
            self.app.mod_flow(
                command=self.app.ofproto.OFPFC_DELETE,
                table_id=constants.SFC_MPLS_DISPATCH_TABLE,
    ...
    def _pp_to_bucket(self, pp):
        if (
            pp.correlation_mechanism == sfc.CORR_MPLS or
            not pp.ingress_port.is_local
        ):
        ...
        actions = [
            self.app.parser.OFPActionSetField(reg7=pp.ingress_port.unique_key),
    ...
    def _pp_ingress_to_network_id(self, pp):
        return pp.ingress_port.lswitch.unique_key
    ...
    def install_dispatch_to_ppg_flows(self, pc, ppg):
        for fc in pc.flow_classifiers:
            ...
            for pp in ppg.port_pairs:
                if pp.correlation_mechanism != sfc.CORR_NONE:
                    continue
            ...
            local_pps = [
                pp for pp in ppg.port_pairs if pp.ingress_port.is_local
            ]
    ...
    def _install_mpls_sf_egress_flows(self, pc, ppg, pp):
        for fc in pc.flow_classifiers:
            self.app.mod_flow(
                ...
                priority=constants.PRIORITY_VERY_HIGH,
                match=self.app.parser.OFPMatch(
                    reg6=pp.egress_port.unique_key,
                    eth_type=ether_types.ETH_TYPE_MPLS,
