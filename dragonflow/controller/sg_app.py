...
class SGApp(df_base_app.DFlowApp):
    ...
    def _get_security_rule_by_addresses_match_item(self, ethertype, flow_direction):
        match_item = ""
    ...
    def _uninstall_security_group_permit_flow_by_direction(self,
        ...
        match_v6 = parser.OFPMatch(eth_type=ether.ETH_TYPE_IPV6, conj_id=conj_id)
        self.mod_flow(
            table_id=table_id,
            match=match_v4,
    ...
    def _update_security_group_rule_flows_by_addresses(self,
        ...
        for added_cidr_item in added_cidr:
            if netaddr.IPNetwork(added_cidr_item).version == utils.ip_version_convertor[ethertype]:
        ...
        for removed_cidr_item in removed_cidr:
            if netaddr.IPNetwork(removed_cidr_item).version == utils.ip_version_convertor[ethertype]:
              for match_item in match_list:
                  parameters_merge = match_item.copy()
                  parameters_merge[ip_match_item] = \
        ...
        if remote_group_id is not None:
            ...
            if aggregate_addresses_range is not None:
                ...
                for aggregate_address in cidr_list:
                    if netaddr.IPNetwork(aggregate_address).version == utils.ip_version_convertor[ethertype]:
                        ...
                        print("Skipping. ip is {}. version is {}".format(aggregate_address, ethertype))
        elif remote_ip_prefix is not None:
            if netaddr.IPNetwork(remote_ip_prefix).version == utils.ip_version_convertor[ethertype]:
                addresses_list = [{
                    ip_match_item: SGApp._get_network_and_mask(
                        remote_ip_prefix
                    )
    ...
    def add_local_port(self, lport):
        secgroups = lport.get_security_groups()
        if not secgroups:
            LOG.warning("lport has no security groups (adding new lport). lport=%s", lport)
            return
