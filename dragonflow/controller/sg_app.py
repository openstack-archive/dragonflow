...
class SGApp(DFlowApp):
    ...
    def switch_features_handler(self, ev):
        ...
        self.install_init_flow_by_direction('ingress')
        self.install_init_flow_by_direction('egress')

    def install_init_flow_by_direction(self, direction):

        parser = self.get_datapath().ofproto_parser
        ofproto = self.get_datapath().ofproto

        # install ct flow to make packet tracked
        if direction == 'ingress':
            pre_table = const.INGRESS_PRE_CONNTRACK_TABLE
            recirc_table = const.INGRESS_CONNTRACK_TABLE
            ct_table = const.INGRESS_CONNTRACK_TABLE
            sg_table = const.INGRESS_SECURITY_GROUP_TABLE
            goto_table = const.INGRESS_DISPATCH_TABLE
        else:
            pre_table = const.EGRESS_PRE_CONNTRACK_TABLE
            recirc_table = const.EGRESS_CONNTRACK_TABLE
            ct_table = const.EGRESS_CONNTRACK_TABLE
            sg_table = const.EGRESS_SECURITY_GROUP_TABLE
        ...
        match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,
                                ct_state=(CT_STATE_TRK | CT_STATE_EST,
                                          CT_STATE_MASK))
        ...
        match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,
                                ct_state=(CT_STATE_TRK | CT_STATE_REL,
                                          CT_STATE_MASK))
        ...
        # inv state, drop
        match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,
                                ct_state=(CT_STATE_TRK | CT_STATE_INV,
                                          CT_STATE_MASK))
        self.mod_flow(
             self.get_datapath(),
             table_id=ct_table,
        ...
        match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,
                                ct_state=(CT_STATE_TRK | CT_STATE_NEW,
                                          CT_STATE_MASK))
        goto_inst = parser.OFPInstructionGotoTable(
                    sg_table)
        inst = [goto_inst]
    ...
    def remove_local_port(self, lport):
        ...
        for secgroup_id in secgroups:
            if self.secgroup_associate_ports.get(secgroup_id) is not None:
                self.secgroup_associate_ports[secgroup_id]. \
                                        discard(lport.get_id())
            secgroup = self.db_store.get_security_group(secgroup_id)
            for rule in secgroup.rules:
                direction = rule.direction
                rule_id = self.get_security_rule_mapping(rule.id)
                if direction == 'egress':
                    secgroup_table = const.EGRESS_SECURITY_GROUP_TABLE
                    match = parser.OFPMatch()
                    match.set_in_port(ofport)
                else:
                    secgroup_table = const.INGRESS_SECURITY_GROUP_TABLE
                    match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,
                                            tunnel_id_nxm=tunnel_key)
                # if in_gress, match should set to tun_id
                msg = parser.OFPFlowMod(
                    datapath=self.get_datapath(),
                    cookie=rule_id,
                    cookie_mask=COOKIE_FULLMASK,
                    table_id=secgroup_table,
            ...
            if secrules is not None:
                for rule_id in secrules:
                    rule_info = secrules[rule_id]
    ...
    def add_local_port(self, lport):
        ...
        if secgroups is None:
            # TODO(dingbo) install jump flow in pre ct table
            return
        ...
        for secgroup_id in secgroups:
            ...
            for rule in secgroup.rules:
                ...
                if direction == 'egress':
                    ...
                    match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,
                                            tunnel_id_nxm=tunnel_key)
                actions = []
                actions.append(parser.NXActionConjunction(clause=0,
            ...
            if secrules is not None:
                for rule_id in secrules:
                    ...
                    actions = []
                    actions.append(parser.
                                   NXActionConjunction(clause=dimensions - 1,
                                                       n_clauses=dimensions,
                                                       id_=rule_id))
                    action_inst = self.get_datapath(). \
                        ofproto_parser.OFPInstructionActions(
    ...
    def add_security_group_rule(self, secgroup, secgroup_rule):
        ...
        if ethertype == 'IPv4':
            ...
            if protocol is not None:
                if protocol == 'icmp':
                    ...
                    if port_range_min is not None:
                        for port in range(int(port_range_min),
                                          int(port_range_max)):
                            match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,
                                                    ip_proto=6,
                                                    tcp_dst=port)
            ...
            if remote_group_id is not None:
                ...
                associate_ports = self.secgroup_associate_ports. \
                    get(remote_group_id)
        ...
        # for local port, install first dimension flow
        ...
        actions = []
        actions.append(parser.NXActionCT(actions=[], alg=0, flags=1,
                                         recirc_table=recirc_table,
                                         zone_ofs_nbits=15,
                                         zone_src=CT_ZONE_REG))
        action_inst = self.get_datapath().ofproto_parser.OFPInstructionActions(
            ofproto.OFPIT_APPLY_ACTIONS, actions)
        inst = [action_inst]
        self.mod_flow(
