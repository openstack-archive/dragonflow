...
class SGApp(df_base_app.DFlowApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.secgroup_aggregate_addresses = collections.defaultdict(list)
        self.secgroup_ip_refs = collections.defaultdict(set)
        self.register_local_cookie_bits(COOKIE_NAME, 32)

    @staticmethod
    def _get_cidr_changes_after_removing_addresses(cidr_list, address_list):
        ...
        for removed_ip in address_list:
            removed_ip_value = int(netaddr.IPAddress(removed_ip))
            new_cidr_set, temp_added_cidr, temp_removed_cidr = \
                SGApp._remove_one_address(new_cidr_list, removed_ip_value)
    ...
    @staticmethod
    def _get_cidr_changes_after_adding_addresses(cidr_list, address_list):
        """cidr_list - IPNetwork list
           address_list - IPAddress or string list
        """
        added_cidr = []
        ...
        for added_ip in address_list:
            added_ip_value = int(netaddr.IPAddress(added_ip))
            new_cidr_list, temp_added_cidr, temp_removed_cidr = \
                SGApp._add_one_address(new_cidr_list, added_ip_value)
            added_cidr.extend(temp_added_cidr)
            removed_cidr.extend(temp_removed_cidr)
    ...
    @staticmethod
    def _get_cidr_changes_after_updating_addresses(cidr_list, addresses_to_add,
                                                   addresses_to_remove):
        """cidr_list - IPNetwork list
        ...
        for removed_ip in addresses_to_remove:
            ...
            new_cidr_set, temp_added_cidr, temp_removed_cidr = \
                SGApp._remove_one_address(
                    new_cidr_list, removed_ip_value
                )
            added_cidr.extend(temp_added_cidr)
            removed_cidr.extend(temp_removed_cidr)
    ...
    def _if_cidrs_can_be_merged(cidr1, cidr2):
        if (cidr1.last + 1 == cidr2.first and
                cidr1.last - cidr1.first == cidr2.last - cidr2.first):
            return True
    ...
    def _remove_one_address(cidr_list, address_to_remove):
        ...
        for index in range(len(cidr_list)):
            ...
            if address_to_remove in cidr_item:
                ...
                added_cidr = netaddr.cidr_exclude(address_to_remove)
    ...
    def _add_one_address(cidr_list, address_to_add):
        ...
        for index in range(len(cidr_list)):
            cidr_item = cidr_list[index]
            if cidr_item.last >= address_to_add:
                if cidr_item.first <= address_to_add:
                    return cidr_list, [], []
                position = index
                break
        ...
        current_cidr = netaddr.IPNetwork((address_to_add, 32))
        removed_cidr = []
        continue_flag = True
        while continue_flag:
            continue_flag = False
            if left_position >= 0:
                ...
                if SGApp._if_cidrs_can_be_merged(left_item, current_cidr):
            ...
            if right_position < len(cidr_list):
                ...
                if SGApp._if_cidrs_can_be_merged(current_cidr, right_item):
    ...
    def _associate_secgroup_lport_addresses(self, secgroup_id, lport):
        ...
        new_cidr_list, added_cidr, removed_cidr = \
            SGApp._get_cidr_changes_after_adding_addresses(
                addresses,
                added_ips,
            )
        self.secgroup_aggregate_addresses[secgroup_id] = new_cidr_list
