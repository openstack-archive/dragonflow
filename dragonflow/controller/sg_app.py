...
class SGApp(df_base_app.DFlowApp):
    ...
    def _get_delete_conntrack_entries_filter_list(self, port_info, rule,
        ...
        for port_ip in port_info['removed_ips']:
            ...
            if remote_address_list:
                for remote_address in remote_address_list:
                    ...
                    entries_filter_list.append(entries_filter_tmp)
            else:
                entries_filter_list.append(entries_filter)

        return entries_filter_list
    ...
    def _delete_conntrack_entries_by_rule(self, rule, filter_port_info=None,
        ...
        if filter_port_info:
            ...
            for port_id in associating_port_ids:
                ...
                associating_ports_info.append({'remoted_ips': removed_ips,
                                               'zone_id': zone_id})

        for port_info in associating_ports_info:
            entries_filter_list = \
                self._get_delete_conntrack_entries_filter_list(
                    port_info, rule, remote_address_list)
            for entries_filter in entries_filter_list:
    ...
    def _delete_conntrack_entries_by_local_port_info(
        ...
        local_port_info = {'remoted_ips': removed_ips, 'zone_id': zone_id}
