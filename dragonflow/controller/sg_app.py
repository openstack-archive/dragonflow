...
class SGApp(DFlowApp):
    ...
    def switch_features_handler(self, ev):
        ...
        actions.append(parser.NXActionCT(actions=[], alg=0, flags=0,
            recirc_table=const.EGRESS_CT_TABLE, zone_ofs_nbits=15,
        ...
        self.mod_flow(
            self.get_datapath(),
            inst=inst,
            table_id=const.EGRESS_PRE_CT_TABLE,
            priority=const.PRIORITY_MEDIUM,
            match=match)

        actions.append(parser.NXActionCT(actions=[], alg=0, flags=0,
            recirc_table=const.INGRESS_CT_TABLE, zone_ofs_nbits=15,
            zone_src=METADATA_REG))
        action_inst = self.get_datapath().ofproto_parser.OFPInstructionActions(
            ofproto.OFPIT_APPLY_ACTIONS, actions)
        self.mod_flow(
            self.get_datapath(),
            inst=inst,
            table_id=const.INGRESS_PRE_CT_TABLE,
            priority=const.PRIORITY_MEDIUM,
            match=match)

        # est state, pass
        match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,
                                ct_state=(CT_STATE_TRK | CT_STATE_EST, 0x3f))
        ...
        self.mod_flow(
             self.get_datapath(),
             inst=inst,
             table_id=const.EGRESS_CT_TABLE,
             priority=const.PRIORITY_MEDIUM,
             match=match)
        ...
        self.mod_flow(
             self.get_datapath(),
             inst=inst,
             table_id=const.INGRESS_CT_TABLE,
             priority=const.PRIORITY_MEDIUM,
             match=match)

        # rel state, pass
        match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,
                ct_state=(CT_STATE_TRK | CT_STATE_REL, 0x3f))
        self.mod_flow(
             self.get_datapath(),
             inst=inst,
             table_id=const.EGRESS_CT_TABLE,
        ...
        # inv state, drop
        match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,
                                ct_state=(CT_STATE_TRK | CT_STATE_INV, 0x3f))
        self.mod_flow(
             self.get_datapath(),
             table_id=const.EGRESS_CT_TABLE,
             priority=const.PRIORITY_MEDIUM,
        ...
        match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,
                                ct_state=(CT_STATE_TRK | CT_STATE_NEW, 0x3f))
        goto_inst = parser.OFPInstructionGotoTable(
