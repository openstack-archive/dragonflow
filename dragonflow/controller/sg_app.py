...
class SGApp(df_base_app.DFlowApp):
    ...
    def _install_security_group_flows(self, security_group_id):
        ...
        secgroup = self.db_store2.get_one(SecurityGroup(id=security_group_id))
    ...
    def _get_secgroup_conj_id_and_priority(self, secgroup_id):
        sg = self.db_store2.get_one(SecurityGroup(id=secgroup_id))
    ...
    @df_base_app.register_event(SecurityGroup, model_constants.EVENT_CREATED)
    def add_security_group(self, secgroup):
        print("adding rules for new group")
        for new_rule in secgroup.rules:
            self.add_security_group_rule(secgroup, new_rule)

    @df_base_app.register_event(SecurityGroup, model_constants.EVENT_UPDATED)
    def update_security_group(self, new_secgroup, old_secgroup):
        print("updating rules for new group")
        new_secgroup_rules = new_secgroup.rules
        old_secgroup_rules = old_secgroup.rules
        print("new rules = {}".format(new_secgroup_rules))
        print("old rules = {}".format(old_secgroup_rules))
        ...
        for old_rule in old_secgroup_rules:
            print("removing {}".format(old_rule.id))
            self.remove_security_group_rule(old_secgroup, old_rule)

    @df_base_app.register_event(SecurityGroup, model_constants.EVENT_DELETED)
    def delete_security_group(self, secgroup):
        print("deleting rules for new group")
        for new_rule in secgroup.rules:
            self.add_security_group_rule(secgroup, new_rule)

    @df_base_app.register_event(SecurityGroupRule,
                                model_constants.EVENT_CREATED)
    def add_security_group_rule(self, secgroup, secgroup_rule):
        secgroup_id = secgroup.id
        print("in add secgroup rule")
        if self._is_sg_not_associated_with_local_port(secgroup_id):
