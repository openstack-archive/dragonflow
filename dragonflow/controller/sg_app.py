...
class SGApp(DFlowApp):
    ...
    def _add_one_address(cidr_array, address):
        ...
        if position is None:
            left_array = cidr_array[:len(cidr_array)]
            right_array = []
        else:
        ...
        while continue_flag:
            ...
            if len(left_array) != 0:
                ...
                merge_succeed, new_mask = SGApp._try_merge_cidr(current_prefix,
                                                                current_mask,
                                                                left_item,
                                                                full_mask)
                if merge_succeed:
    ...
    def _install_security_group_permit_flow_by_direction(self,
        ...
        if conj_id is None:
            LOG.error(_LI("the conj_id of the security group (%s) is "
                          "none", security_group_id))
            return
    ...
    def _install_security_group_rule_flows(self, secgroup, secgroup_rule):
        ...
        if ethertype == 'IPv4':
            ...
            if remote_group_id is not None:
                ...
                addresses_list = [{
                    ipv4_match_item: SGApp._get_network_and_mask(
                        SGApp._get_cidr_match(remote_ip_prefix)
                    )
                }]
    ...
    def _uninstall_security_group_rule_flows(self, secgroup_rule):
        ...
        rule_id = self._get_security_rule_mapping(secgroup_rule.id)
        if rule_id is None:
            LOG.error(_LI("the rule_id of the security group rule (%s) "
                          "is none", rule_id))
            return
    ...
    def _install_env_init_flow_by_direction(self, direction):
        ...
        # defaults of pre-sg-table to pass packet
    ...
    def _allocate_security_group_id(self, lgroup_id):
        ...
        LOG.info(_LI("allocate a number (%s) to the security group (%s)",
                     security_id, lgroup_id))
    ...
    def _release_security_group_id(self, lgroup_id):
        ...
        LOG.info(_LI("release the allocated number (%s) of the security "
                     "group (%s)", security_id, lgroup_id))
    ...
    def _get_secgroup_conj_id_and_priority(self, lgroup_id):
        security_id = self.secgroup_mappings.get(lgroup_id)
        if security_id is not None:
            return security_id, (2 + security_id)
        return None, None

    def remove_local_port(self, lport):
        if self.get_datapath() is None:
            LOG.error(_LI("datapath is none"))
            return
    ...
    def add_security_group_rule(self, secgroup, secgroup_rule):
        LOG.info(_LI("add a rule (%s) to the security group (%s)",
                     secgroup_rule.id, secgroup.name))
        if self.get_datapath() is None:
            LOG.error(_LI("datapath is none"))
            return
    ...
    def remove_security_group_rule(self, secgroup, secgroup_rule):
        LOG.info(_LI("remove a rule (%s) form the security group (%s)",
                     secgroup_rule.id, secgroup.name))
        if self.get_datapath() is None:
            LOG.error(_LI("datapath is none"))
            return
