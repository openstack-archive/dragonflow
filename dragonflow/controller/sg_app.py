    ...
    def remove_local_port(self, lport):
        ...
        for secgroup_id in secgroups:
            if self.secgroup_associate_ports.get(secgroup_id) is not None:
                self.secgroup_associate_ports[secgroup_id].remove(lport.get_id())
            ...
            if secrules is not None:
                for secrule in secrules:
                    ...
                    match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,
                            ipv4_dst=ip)
                    rule_id = self.get_security_rule_mapping(secrule.id)
                    msg = parser.OFPFlowMod(
                        datapath=self.get_datapath(),
                        cookie=rule_id,
                        cookie_mask=COOKIE_FULLMASK,
        ...
        for secgroup_id in secgroups:
            ...
            if secrules is not None:
                for secrule in secrules:
                    ...
                    security_group_table = const.EGRESS_SECURITY_GROUP_TABLE \
                        if direction == 'egress' else const.INGRESS_SECURITY_GROUP_TABLE
                    dimensions = self.get_security_rule_dimension(secrule)
                    match = parser.OFPMatch(eth_type=ether.ETH_TYPE_IP,
                            ipv4_dst=ip)
                    actions = []
                    rule_id = self.get_security_rule_mapping(secrule.id)
        ...
        if ethertype == 'IPv4':
            ...
            if remote_group_id is not None:
                ...
                if associate_rules is None:
                    self.remote_secgroup_ref[remote_group_id] = set([secgroup_rule])
                else:
                    associate_rules.add(secgroup_rule)
