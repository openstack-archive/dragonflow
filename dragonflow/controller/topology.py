...
class Topology(object):
    ...
    def _check_ovs_port_integrity(ovs_port):
        if (ovs_port.get_ofport() is None) or (ovs_port.get_type() is None):
            return False
        if (ovs_port.get_type() == OvsPort.TYPE_VM) and \
    ...
    def _vm_port_added(self, ovs_port):
        ...
        try:
            ...
            LOG.error(_LE('App failed to process vm port online event %s')
                      % str(lport))
            LOG.exception(_LE("Exception occurred:"))
        finally:
    ...
    def _vm_port_deleted(self, ovs_port):
        ...
        assert lport is not None, "Lport:{id} not found".format(id=lport_id)
        ...
        try:
            ...
            #                             status='DOWN')
            self.logical_to_ovs_port_mapping.pop(lport_id)
