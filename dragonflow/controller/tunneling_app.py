...
class TunnelingApp(df_base_app.DFlowApp):
    ...
    def _new_network_flow(self, lport, network_id, network_type):
        LOG.info(_LI("adding new %(net_type)s network %(network_id)s"),
                {'net_type': network_type,
                 'network_id': network_id})
    ...
    def _add_egress_dispatch_flow(self, lport, segmentation_id):
        ...
        LOG.info(_LI("set egress dispatch flow %(seg)s peer %(remote_ip)s"),
                {'seg': segmentation_id,
                 'remote_ip': remote_ip})
    ...
    def _eval_flow_actions(self, network_id, network_type, segmentation_id,
                           port_count, command):
        inst = None
        if port_count == 0:
    ...
    def _modify_egress_bum_flow(self,
        ...
        port_count = self.local_networks.get_remote_port_count(
                network_id=network_id,
                network_type=network_type)
        inst, command = self._eval_flow_actions(network_id, network_type,
                segmentation_id, port_count, command)
        self.mod_flow(
            inst=inst,
            table_id=const.EGRESS_TABLE,
            command=command,
