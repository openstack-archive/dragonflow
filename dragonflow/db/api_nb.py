...
class NbApi(object):
    ...
    def initialize(self, db_ip='127.0.0.1', db_port=4001):
        ...
        self.publisher = self._PulisherCRUDHelper(self, db_models.Publisher)
    ...
    class _CRUDHelper(object):
        ...
        def create(self, id, topic, notify=True, **columns):
            # Allocate unique key if our class needs one
            if issubclass(self.model, db_models.NbDbObjectWithUniqueKey):
        ...
        def update(self, id, topic, notify=True, **columns):
            ...
            # original.update(columns)
            for key, value in six.iteritems(columns):
                if value == const.ATTR_NOT_SPECIFIED:
                    continue
                original[key] = value

            obj_json = self._serialize_object(id, topic, **original)
        ...
        def _add_element(self, id, topic, new_version, elem_name, nested_obj):
            obj = self._get_raw(id, topic)
            obj['version'] = new_version
            obj.setdefault(elem_name, []).append(nested_obj)
            self.update(id, topic, **obj)
    ...
    class _SecurityGroupCRUDHelper(_CRUDHelper):
    ...
    class _LswitchCRUDHelper(_CRUDHelper):
    ...
    def _LportCRUDHelper(_CRUDHelper):
        def get_all(self, topic=None):
    ...
    class _FloatingipCRUDHelper(_CRUDHelper):
        def create(self, id, topic, **columns):
            notify = columns.get('port_id') is not None
            super(NbApi._FloatingipCRUDHelper.self).create(
                id, topic, notify=notify, **columns)

    class _PulisherCRUDHelper(_CRUDHelper):
        @classmethod
        ...
        def get_all(self, topic=None):
            publishers = super(NbApi._PulisherCRUDHelper, self).get_all(topic)
