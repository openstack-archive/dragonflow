...
class NbApi(object):
    ...
    def apply_db_change(self, table, key, action, value):
        ...
        if model_class:
            if action == 'delete':
                # FIXME temporary adaptation to convert new model types
                try:
                    # New model
                    arg = model_class(id=key)
                except TypeError:
                    # Old model
                    arg = key
            else:
                try:
                    # New model
                    arg = model_class.from_json(value)
    ...
    def update(self, obj):
        """Update the provided object in the database and publich an event
           about its creation.
        ...
        for key, _ in obj:
            new_attr = getattr(obj, key)
            if new_attr in (None, const.ATTR_NOT_SPECIFIED):
                continue

            setattr(full_obj, key, new_attr)
    ...
    def delete(self, obj):
        ...
        obj.on_delete_pre()
        try:
            self.driver.delete_key(model.table_name, obj.id, obj.topic)
    ...
    def get_all(self, model, topic=None):
        ...
        all_objects = [
            model.from_json(e)
            for e in self.driver.get_all_entries(model.table_name, topic)
        ]
