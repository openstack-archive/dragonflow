...
class DbUpdate(object):
    ...
    def __str__(self):
        return"Action:%s, Table:%s" % (self.action, self.table)
...
class NbApi(object):
    ...
    def _send_db_change_event(self, table, key, action, value):
        if self.use_pubsub:
            update = DbUpdate(table, key, action, None)
    ...
    def _read_db_changes_from_queue(self):
        while True:
            ...
            try:
                ...
                if self.next_update.action != 'delete':
