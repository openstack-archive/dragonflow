...
class NbApi(object):
    ...
    def __init__(self, db_driver, use_pubsub=False, is_neutron_server=False):
        ...
        self._db_change_callback = self._dummy_db_change_callback
    ...
    def set_db_change_callback(self, db_change_callback):
        if self.use_pubsub and not self.is_neutron_server:
            self._db_change_callback = db_change_callback
    ...
    def db_change_callback(self, table, key, action, value, topic=None):
        self._db_change_callback(table, key, action, value, topic)

    def _dummy_db_change_callback(self, table, key, action, value, topic):
        pass
