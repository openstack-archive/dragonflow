...
class PublisherAgent(object):
    ...
    def send_event(self, table, key, action, value):
        ...
        if self.is_daemonize:
            ...
            #NOTE (gampel) Currently Controllers send only
            #Notification for Chassis, this will happen only
            #once per compute node registration If in the future
            #controller send event more often we should call
            #demonise on controller code as well
