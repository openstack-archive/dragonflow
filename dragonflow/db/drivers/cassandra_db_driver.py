...
class CassandraDbDriver(db_api.DbApi):
    ...
    def initialize(self, db_ip, db_port, **args):
        ...
        lb_policy = self.LOAD_BALANCING_OPT_MAPPINGS[
            self.config.load_balancing]
        consistency = self.CONSISTENCY_OPT_MAPPINGS[
    ...
    def create_table(self, table):
        self.session.execute("CREATE TABLE IF NOT EXISTS %s "
                             "(key str PRIMARY KEY, value text)" % table)
