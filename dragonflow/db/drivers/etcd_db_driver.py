...
class EtcdDbDriver(db_api.DbApi):
    ...
    def delete_key(self, table, key, topic=None):
        # try:
        LOG.error("trying to delete key {}/{}".format(table, key))
        deleted = self.client.delete('/' + table + '/' + key)
        if deleted:
            LOG.error("key {}/{} deleted".format(table, key))
        else:
    ...
    def get_all_keys(self, table, topic=None):
        res = []
        directory = self.client.get_prefix("/" + table, metadata=True)
        for entry in directory:
    ...
    def _allocate_unique_key(self, table):
        ...
        prev_value = 0
        try:
            prev_value = int(self.client.get(key).pop())
