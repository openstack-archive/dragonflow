...
class OvsdbSwitchApi(api_vswitch.SwitchApi):
    ...
    def __init__(self, ip, open_flow_app, protocol='tcp', port='6640', timeout=10):
    ...
    def initialize(self):
        ...
        remote = self.ip + ":" + self.port
        ovsdb_monitor = OvsdbMonitor(remote, self.open_flow_app)
        ovsdb_monitor.start()
...
class OvsdbMonitor(object):
    ...
    def __init__(self, remote, open_flow_app):
    ...
    def handle_update(self, table_update):
        ...
        for row_uuid, table_row in table_rows.iteritems():
            ...
            if not old and not new:
                ...
                self.open_flow_app.add_new_interface()
            elif not new:
                # delete a old interface
                _interface = api_vswitch.LocalInterface()
                _interface.uuid = row_uuid
                self.parse_interface(_interface, old)
                self.open_flow_app.delete_old_interface()
            else:
                # update a exist interface
                _interface = api_vswitch.LocalInterface()
                _interface.uuid = row_uuid
                self.parse_interface(_interface, new)
                self.open_flow_app.udpate_interface()
