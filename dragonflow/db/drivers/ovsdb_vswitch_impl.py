...
class OvsdbMonitor(object):
    ...
    def connect_ovsdb(self):
        ...
        while unconnected:
            try:
                ...
                LOG.error("could not connect to ip:%s port:%s",
                          self.ip, self.port)
                time.sleep(3)
    ...
    def parse_interface(self, _interface, input_dict):
        ...
        if interface_type == OvsdbMonitor.TYPE_UNKNOW_PORT:
            LOG.debug("receive unknow interface:", input_dict)
            return
    ...
    def handle_message(self):
        while True:
            ...
            retval = self.wait_for_parser()
            if retval == -1:
                LOG.error("exception happened when read from socket!")
                return
