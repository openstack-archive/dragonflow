...
class OvsdbSwitchApi(api_vswitch.SwitchApi):
    ...
    @staticmethod
    def _check_mac_in_use(port_name, mac_in_use):
        if mac_in_use is None:
            LOG.warning(_LW("Can't find mac_in_use for port %s."), port_name)
            return False
        try:
            valid_mac = netaddr.valid_mac(mac_in_use)
        except Exception:
            valid_mac = False

        if not valid_mac:
            LOG.warning(
                _LW("mac_in_use %(mac)s for port %(port)s is invalid."),
                {'mac': mac_in_use, 'port': port_name})
            return False
