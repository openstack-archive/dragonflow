...
class OvsdbSwitchApi(api_vswitch.SwitchApi):
    ...
    def initialize(self):
        ...
        self.ovsdb = connection.Connection(db_connection,
                                           self.timeout,
                                           self.db_name)
        tbl = constants.OVS_INTERFACE
        self.nb_api.db_change_callback(tbl, None, "sync_started", None, None)
        self.ovsdb.start()
        self.idl = self.ovsdb.idl

        self.ovsdb_monitor = OvsdbMonitor(self.nb_api, self.idl)
        self.idl.notify = self.ovsdb_monitor.notify
        self.nb_api.db_change_callback(tbl, None, "sync_finished", None, None)
