...
class RAMCloud(object):
    ...
    def delete_rr(self, table_id, key, reject_rules):
        ...
        self.hook()
        s = so.rc_remove(self.client, table_id, get_key(key),
                         get_keyLength(key), ctypes.byref(reject_rules),
    ...
    def write_rr(self, table_id, key, data, reject_rules):
        got_version = ctypes.c_uint64()
        self.hook()
        s = so.rc_write(self.client, table_id, get_key(key),
                        get_keyLength(key), data, len(data),
