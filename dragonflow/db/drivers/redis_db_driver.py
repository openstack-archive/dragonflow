    ...
    def populate_cluster(self):
        for node in self._configured_nodes:
            ...
            try:
                ...
                LOG.exception('Error getting cluster slots from %s:%s',
                              node.ip, node.port)
                continue
            for slot_info in slots:
                ...
                for slot in range(range_begin, range_end + 1):
...
class RedisDbDriver(db_api.DbApi):
    ...
    @staticmethod
    def _args_to_cmd(command, key, args=()):
        if args is not None and len(args) > 0:
            return '%s %s %s' % (command, key, ' '.join(args))
        else:
            return '%s %s' % (command, key)
        ...
        while retry > 0:
            ...
            try:
                if ask:
                    ...
                    ask = False
                return node.client.execute_command(command_str)
