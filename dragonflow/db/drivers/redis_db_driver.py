...
class RedisDbDriver(db_api.DbApi):
    ...
    def _get_key_notopic(self, table, key):
        ...
        result = []
        self._bulk_operation(table, '*', 'GET', key_pattern=key,
                                    entry_cb=add_key)
        n_keys = len(result)
        if n_keys != 1:
    ...
    def _query_info(self, table, topic, key='*'):
        if topic is None:
            # ask all nodes
            pattern = self._key_name(table, '*', key)
            nodes = self._cluster.nodes
        else:
            # ask a specific node
            pattern = self._key_name(table, topic, key)
    ...
    def get_all_keys(self, table, topic=None):
        ...
        for node in nodes:
            retry = 0
            while retry < self.RETRY_COUNT:
                LOG.debug('Getting all keys from table %s topic %s retry %d',
                          table, topic or 'None', retry)
                try:
