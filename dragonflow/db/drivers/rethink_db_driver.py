...
class RethinkDbDriver(db_api.DbApi):
    ...
    def single_feed(self, table, callback):
        import time
        print ("Feed Table name: " + table +
               " on host: " + self.db_ip + " on port: " + self.db_port)
...
if __name__ == "__main__":
    ...
    test_data = '{"external_ids": {"neutron:router_name": "router1"},' \
                ' "name": "neutron-8b8a2dd9-698d-4fc3-aba3-379c8770d8af",' \
                '"ports": [{"network": "10.0.0.1/24", "lswitch":' \
                ' "neutron-8c46938d-4201-4223-abb1-8b4830ea6dcc", ' \
                '"mac": "fa:16:3e:e2:16:63",' \
                '"tunnel_key": 2, "lrouter": ' \
                '"neutron-8b8a2dd9-698d-4fc3-aba3-379c8770d8af", "name"' \
                ': "3fd0768f-2acd-4e5d-bba9-21be7e470571"},' \
                '{"network": "fd63:2be9:34d8::1/64", "lswitch": ' \
                '"neutron-8c46938d-4201-4223-abb1-8b4830ea6dcc", ' \
                '"mac": "fa:16:3e:f1:d1:f1",' \
                '"tunnel_key": 4, "lrouter": ' \
                '"neutron-8b8a2dd9-698d-4fc3-aba3-379c8770d8af", ' \
                '"name": "55f20f8d-e13b-436a-87cf-0bddf612561f"}]}'
    test_data_1 = '{"external_ids": {"neutron:router_name": "router3"}}'
    test_data_2 = '{"external_ids": {"neutron:router_name": "router4"}, ' \
                  '"name": "neutron-9k8b2cc3-698d-4fc3-aba3-379c8770d8af",' \
                  '"ports": [{"network": "10.0.0.1/24", "lswitch": ' \
                  '"neutron-8c46938d-4201-4223-abb1-8b4830ea6dcc", ' \
                  '"mac": "fa:16:3e:e2:16:63",' \
                  '"tunnel_key": 2, "lrouter": ' \
                  '"neutron-8b8a2dd9-698d-4fc3-aba3-379c8770d8af", ' \
                  '"name": "3fd0768f-2acd-4e5d-bba9-21be7e470571"},' \
                  '{"network": "fd63:2be9:34d8::1/64", "lswitch": ' \
                  '"neutron-8c46938d-4201-4223-abb1-8b4830ea6dcc", ' \
                  '"mac": "fa:16:3e:f1:d1:f1",' \
                  '"tunnel_key": 4, "lrouter": ' \
    ...
    print ("######################## TEST NUMBER:" +
           str(test_nmb) + " ###################################")
    print ("Creating 2 entries with UUID "
           "neutron-8b8a2dd9-698d-4fc3-aba3-379c8770d8af and"
           " neutron-9k8b2cc3-698d-4fc3-aba3-379c8770d8af")
    RD.create_key('dragonflow', '3fd0768f-2acd-4e5d-bba9-21be7e470571',
                  test_data)
    RD.create_key('dragonflow', '3fd0768f-2acd-4e5d-bba9-21be7e470571',
                  test_data_2)
    output = RD.get_all_entries('dragonflow')
    ...
    print ("Delete Value by Key "
           "neutron-9k8b2cc3-698d-4fc3-aba3-379c8770d8af and print list")
    ...
    print ("######################## TEST NUMBER:" +
           str(test_nmb) + " ###################################")
    print ("Update Entry by Key "
          "neutron-8b8a2dd9-698d-4fc3-aba3-379c8770d8af "
           "(router1-> router3): ")
    RD.set_key('dragonflow', "neutron-8b8a2dd9-698d-4fc3-aba3-379c8770d8af",
               test_data_1)
    output = RD.get_key('dragonflow',
                        "neutron-8b8a2dd9-698d-4fc3-aba3-379c8770d8af")
    print ("Updated Value")
    ...
    print ("###########STATIC TESTS DONE: " +
           str(test_nmb) + " of 6, STARTS S/N TESTS ####################")
