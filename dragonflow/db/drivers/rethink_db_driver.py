...
class RethinkDbDriver(db_api.DbApi):
    ...
    def get_key(self, table, key):
        ret_val = jsonutils.dumps(self.client.db('dragonflow').table(table).
    ...
    def delete_key(self, table, key):
        self.client.db('dragonflow').table(table).get(key).delete().\
            run(self.conn)

    def get_all_entries(self, table):
        res = []
        cursor = self.client.db('dragonflow').table(table).run(self.conn)
        for entry in cursor:
    ...
    def get_all_keys(self, table):
        res = []
        cursor = self.client.db('dragonflow').table(table).pluck("name").\
            run(self.conn)
        for entry in cursor:
    ...
    def register_notification_callback(self, callback, prefix=""):
        all_tables = self.client.db('dragonflow').table_list().run(self.conn)
