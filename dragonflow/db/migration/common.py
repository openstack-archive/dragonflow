...
import dragonflow.db.migration
...
def get_sorted_all_version_modules():
    ...
    path = dragonflow.db.migration.__path__[0]
    current_mod_name = __name__.split('.')[-1]
    mods = []
    for f in os.listdir(path):
        mod_name, file_ext = os.path.splitext(os.path.split(f)[-1])
        mod_path = os.path.join(path, f)
        if (file_ext.lower() == '.py' and not mod_name == current_mod_name
