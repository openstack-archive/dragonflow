...
class Service(mf.ModelBase):
    ...
    def on_create_pre(self):
        chassis_id = self.chassis
        if not isinstance(self.chassis, six.string_types):
            chassis_id = self.chassis.id
        self.id = df_utils.generate_uuid(chassis_id, self.binary)
    ...
    def _get_instance_of_bin_on_chassis(cls, nb_api, chassis, binary):
        ...
        """
        return nb_api.get(cls(chassis=chassis, binary=binary),
