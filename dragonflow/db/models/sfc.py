...
class FlowClassifier(mf.ModelBase,
    ...
    def is_classification_local(self):
        ...
        available only after classification app set it, so there is no use
        installing it on other hosts.

        For classification on dest lport, we match using reg7, which is set
        on any host doing L2 for the packet. We can classify the packet right
        away on any of the hosts and forward it to the first SF, saving 2 hops
        of going to dest node then to the first SF.
    '''
    ...
    def is_dispatch_local(self):
        ...
        created because no one will set reg6 again.
