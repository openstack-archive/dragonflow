...
class NbDbUniqueKeyMixin(models.Base):
...
class Chassis(NbDbModelBase):
    ...
    tunnel_type = EnumField(['list', 'of', 'supported', 'types'],
                            required=True)
...
class LogicalPort(NbDbModelBase):
    ...
    subnets = fields.ListField(
        functools.partial(RefWrapper, Subnet),  # Bind Subnet class to ctor
    )
    chassis = Ref(Chassis)
    lswitch = Ref(LogicalSwitch)
    security_groups = fields.ListField(
        functools.partial(RefWrapper, 'SecurityGroup'),
    )
