...
def _acquire_lock(context, oid):
    ...
    while(wait_lock_retries > 0):
        try:
            ...
            with session.begin():
                ...
                #TODO(nick-ma-z): deal with multiple records error
    ...
    # NOTE(nick-ma-z): The lock cannot be acquired. We log this info
    # but don't raise the exception here to block the operation.
    # This behavior will be revisited later.
    LOG.warning(_LW("Acquiring the lock for object %(oid)s failed in "
                    "session %(sid)."), {'oid': oid, 'sid': sid})
...
def _delete_db_row(session, oid):
    row = session.query(models.DFLockedObjects).filter_by(
        object_uuid=oid).one()
    session.delete(row)
    session.flush()
