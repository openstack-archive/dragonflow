...
def _acquire_lock(context, oid):
    ...
    while(max_tries > 0):
        try:
            with db_api.autonested_transaction(context.session):
                LOG.info(_LI("Try to get lock for object %s in session %s."
                             % (oid, sid)))
                row = _get_object_with_lock(context.session, oid, False)
                _update_lock(context.session, row, lock=True, session_id=sid)
            LOG.info(_LI("Lock is acquired for object %s in session %s."
                         % (oid, sid)))
            return sid
...
def _release_lock(context, oid, sid):
    ...
    while(max_tries > 0):
        try:
            ...
            LOG.info(_LI("Lock is released for object %s in session %s."
                         % (oid, sid)))
            return
        except sa_exc.NoResultFound:
            time.sleep(WAIT_SECONDS)
