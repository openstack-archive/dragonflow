    ...
    try:
        ...
        with session.begin():
            ...
            if row.lock and timeutils.is_older_than(row.created_at,
                ...
                _lock_free_update(session, id, lock_state=True, is_reset=True)
...
def _lock_free_update(session, id, lock_state=False,
                      session_id=0, is_reset=False):
    ...
    :param session_id: the API session ID
    :type session_id:  string
    :param is_reset:   to determine if the lock needs to be reset
    :type is_reset:    boolean
    :raises:           RetryRequest() if the lock has been acquired
    """
    if not state:
        # acquire lock
