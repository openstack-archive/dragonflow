...
class PortStatus(object):
    ...
    def _write_status_db(self):
        pass

    def _publish_status_event(self, ovs_port):
        port_id = ovs_port.get_iface_id()
        self.send_port_status_event('lport', port_id, 'update', 'up')

    def _server_lock_monitor(self):
        # TODO(kang) This func is for scanning design, based on zmq.
        pass
    ...
    def send_port_status_event(self, table, key, action, value):
        if self.use_pubsub:
            topics = self.nb_api.driver.get_all_key('portstats')
    ...
    # server  code
    def start_subsciber(self, topics):
        self.initialize(self.port_status_callback)
        server_ip = cfg.CONF.df.local_ip
        self.nb_api.register_topic(server_ip)
        # In portstats table, there are key value pairs like this:
        # port_status_192.168.1.10 : 192.168.1.10
        self.nb_api.driver.create_key('portstats', 'port_status_',
                                      server_ip, server_ip)
...
class PortStatusMonitor(TableMonitor):
    def __init__(self, table_name, driver, publisher=None,
        ...
        self._driver.create_key(self.table_name, 'port_status_',
    ...
    def _poll_once(self, old_cache):
        ...
        self._driver.create_key(self.table_name, 'port_status',
                                time.time(), self._server_ip)
