...
from dargonflow._i18n import _LE
...
class PublisherAgentBase(PublisherApi):
    ...
    def send_event(self, update, topic=b'D'):
    ...
    def daemonize(self):
        self.deamon.daemonize(self.run)

    def stop(self):
        self.deamon.daemonize()
...
class DFDeamon(object):

    def __init__(self):
        super(DFDeamon, self).__init__()
    ...
    def daemonize(self, run):
        if self.is_daemonize:
            LOG.error(_LE("all ready aemonize"))
            return
