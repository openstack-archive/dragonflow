...
class DfDvrRouter(dvr_router.DvrRouter):
    ...
    def add_snat_binding_to_controller(self, sn_port, port):
        return
        if sn_port is None:
            LOG.debug("None sn_port")
            return
        LOG.debug("add_snat_binding_to_controller")
        LOG.debug("subnet = " + sn_port['fixed_ips'][0]['subnet_id'])
        LOG.debug("ip = " + sn_port['fixed_ips'][0]['ip_address'])
        LOG.debug("mac = " + sn_port['mac_address'])
        LOG.debug("Printing router")
        LOG.debug(self.router)
        LOG.debug("Printing port")
        LOG.debug(port)
    ...
    def internal_network_added(self, port):
        ...
        self.add_snat_binding_to_controller(sn_port, port)
    ...
    def internal_network_removed(self, port):
        ...
        self.remove_snat_binding_to_controller(sn_port, port)
    ...
    def external_gateway_added(self, ex_gw_port, interface_name):
        ...
        for p in self.internal_ports:
            ...
            self.add_snat_binding_to_controller(gateway, p)
    ...
    def external_gateway_removed(self, ex_gw_port, interface_name):
        ...
        for p in self.internal_ports:
            gateway = self._map_internal_interfaces(p, snat_ports)
            self.remove_snat_binding_to_controller(gateway, p)

        if not self._is_this_snat_host():
            # no centralized SNAT gateway for this node/agent
            LOG.error("not hosting snat for router: %s", self.router['id'])
            return
