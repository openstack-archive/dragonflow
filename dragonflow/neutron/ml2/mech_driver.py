...
class DFMechDriver(driver_api.MechanismDriver):
    ...
    @lock_db.wrap_db_lock(lock_db.RESOURCE_ALLIEN_CHASSIS)
    def process_agent_update(self, resource, event, trigger, **kwargs):
        ...
        for compute in self.nb_api.get_all(core_models.Chassis):
            self.db_store2.update(compute)

        old_compute = self.db_store2.get_one(
            core_models.Chassis(id=compute_host))

        new_compute = core_models.Chassis(
