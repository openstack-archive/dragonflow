...
class DFMechDriver(driver_api.MechanismDriver,
    ...
    def _create_dhcp_port(self, context, port):
        ...
        return self._create_port_in_nb_api(db_port, sgids)
    ...
    def _handle_delete_subnet_dhcp(self, context, subnet):
        if subnet['enable_dhcp']:
            port = self._get_dhcp_port_for_subnet(context, subnet['id'])
            if port:
                self._delete_subnet_dhcp_port(context, port)
        return
    ...
    def delete_subnet_postcommit(self, context):
        ...
        try:
            self._handle_delete_subnet_dhcp(plugin_context, subnet)
        except Exception as e:
            LOG.exception(e)
            return
    ...
    def _create_port_in_nb_api(self, port, sgids):
    ...
    def create_port_postcommit(self, context):
        ...
        self._create_port_in_nb_api(port, sgids)
