...
class DFMechDriver(driver_api.MechanismDriver):
    ...
    def initialize(self):
        ...
        if cfg.CONF.df.enable_port_status_notifier:
            ...
            port_status_notifier.initialize(self, self.nb_api,
    ...
    def update_port_precommit(self, context):
        ...
        if (cfg.CONF.df.enable_port_status_notifier and
            ...
            ((context.status == n_const.PORT_STATUS_DOWN and
            context.original_status == n_const.PORT_STATUS_UP) or
            (context.status == n_const.PORT_STATUS_UP and
            context.original_status == n_const.PORT_STATUS_DOWN))):
            return None
        port_version = version_db._update_db_version_row(
            context._plugin_context.session, updated_port['id'])
        updated_port['db_version'] = port_version
