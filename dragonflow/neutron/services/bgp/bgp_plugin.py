...
class DFBgpPlugin(service_base.ServicePluginBase,
    ...
    def floatingip_update_callback(self, resource, event, trigger, **kwargs):
        ...
        if port_id:
            ...
            fip_handler = self._add_bgp_speaker_route
        else:
            # Disassociate floatingip
            fip_data = dest
            fip_handler = self._del_bgp_speaker_route
    ...
    def _add_bgp_speaker_route(self, context, bgp_speaker_id, topic, route):
        """Add route to bgp speaker in nb db"""
        ...
        current_routes = {str(r.destination): r for r in bgp_speaker.routes}
        ...
        bgp_speaker.routes = current_routes.values()
        self.nb_api.update(bgp_speaker, skip_send_event=True)

    @lock_db.wrap_db_lock(lock_db.RESOURCE_BGP_SPEAKER)
    def _del_bgp_speaker_route(self, context, bgp_speaker_id, topic, cidr):
        """Delete route from bgp speaker in nd db"""
    ...
    def _update_bgp_speaker_tenant_network_routes(self, context,
        ...
        fip_routes = [r for r in bgp_speaker.routes
                      if r.destination.size == 1]
        bgp_speaker.routes = routes + fip_routes
        self.nb_api.update(bgp_speaker, skip_send_event=True)
    ...
    def _update_bgp_speaker_routes(self, context, bgp_speaker_id, topic):
        ...
        lean_bgp_speaker = bgp.BGPSpeaker(id=bgp_speaker_id,
                                          topic=topic,
                                          routes=prefix_routes + host_routes)
