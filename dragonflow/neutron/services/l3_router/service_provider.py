...
class DfL3ServiceProvider(base.L3ServiceProvider, mixins.LazyNbApiMixin):
    ...
    @log_helpers.log_method_call
    def _create_floatingip(self, resource, event, trigger,
                           context, floating_ip, **kwargs):
        floating_ip = self._l3_plugin.get_floatingip(
            context, floating_ip['id'])
        self.nb_api.create(
            l3.FloatingIp(
                id=floating_ip['id'],
                ...
                lrouter=floating_ip['router_id'],
                lport=floating_ip['port_id'],
                floating_lport=floating_ip['floating_port_id'],
            ),
        )
