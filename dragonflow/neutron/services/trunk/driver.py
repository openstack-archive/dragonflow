...
class DragonflowDriver(base.DriverBase):
    ...
    def _add_subport(self, trunk, subport, df_parent):
        ...
        child = self.nb_api.get(l2.LogicalPort(id=subport.port_id))
        child.chassis = df_parent.chassis
        child.binding_profile['trunk_port_id'] = model.id
        self.nb_api.update(child)
    ...
    def _delete_subport(self, trunk, subport, df_parent):
        ...
        child = self.nb_api.get(l2.LogicalPort(id=subport.port_id))
        child.chassis = None
        child.binding_profile.pop('trunk_port_id', None)
        self.nb_api.update(child)
