...
class DfPortBehindPortDetector(mixins.LazyNbApiMixin):
    ...
    def _detect_port_behind_port(self, a_context,
        ...
        ports with those IPs/MACs. If so, these are nested ports, and create
        ...
        for pair in new_aaps:
            nested_port, segmentation_type = self._find_nested_port(
        ...
        for pair in removed_aaps:
            nested_port, _segmentation_type = self._find_nested_port(
                a_context, pair, updated_port)
            if not nested_port:
    ...
    def _find_nested_port(self, context, pair, port):
        try:
