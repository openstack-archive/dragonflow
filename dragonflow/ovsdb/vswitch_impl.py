...
class OvsApi(object):
    ...
    def create_patch_pair(self, local_bridge, peer_bridge,
        ...
        links = link_mapping.gen_link_mapping(
                    local_link_name,
                    peer_link_name)
        local_bridge_ofport = self.create_patch_port(
                    local_bridge,
                    links[local_bridge],
                    links[peer_bridge])
        peer_bridge_ofport = self.create_patch_port(
                    peer_bridge,
                    links[peer_bridge],
                    links[local_bridge])
        LOG.info("created patch ports "
                 ...
                 {'bridge1': local_bridge,
                  'link1': local_bridge_ofport,
                  'bridge2': peer_bridge,
                  'link2': peer_bridge_ofport})
        return {local_bridge: local_bridge_ofport,
                peer_bridge: peer_bridge_ofport}
