...
class PortPolicyRule(object):
    def __init__(self, pcap_filter, actions):
        self.pcap_filter = pcap_filter
        self.actions = actions
        self.bpf_filter = pcapy.compile(
            pcapy.DLT_EN10MB,
            65535,
            pcap_filter,
            False,
    ...
    def match_packet(self, buf):
        ...
        """
        return (self.bpf_filter.filter(buf) != 0)
...
class PortThread(object):
    ...
    def run(self):
        ...
        while self.is_working:
            try:
                buf = tap.read()
                self.policy.handle_packet(self, buf)
            except Exception as e:
