...
class RyuIPv6Filter(object):
    ...
    def __call__(self, buf):
        pkt = packet.packet.Packet(buf)
        return (pkt.get_protocol(packet.ipv6.ipv6) is not None)
...
class RyuARPReplyFilter(object):
    ...
    def __call__(self, buf):
        pkt = packet.packet.Packet(buf)
        arp = pkt.get_protocol(packet.arp.arp)
        if not arp:
            return False
        return arp.opcode == 2
...
def _get_dhcp_message_type_opt(dhcp_packet):
    for opt in dhcp_packet.options.option_list:
        if opt.tag == packet.dhcp.DHCP_MESSAGE_TYPE_OPT:
...
class RyuDHCPFilter(object):
    ...
    def __call__(self, buf):
        pkt = packet.packet.Packet(buf)
        return (pkt.get_protocol(packet.dhcp.dhcp) is not None)
...
class RyuDHCPPacketTypeFilter(object):
    ...
    def __call__(self, buf):
        pkt = packet.packet.Packet(buf)
        dhcp = pkt.get_protocol(packet.dhcp.dhcp)
        if not dhcp:
            return False
...
class RyuDHCPOfferFilter(RyuDHCPPacketTypeFilter):
    def get_dhcp_packet_type(self):
        return packet.dhcp.DHCP_OFFER


class RyuDHCPAckFilter(RyuDHCPPacketTypeFilter):
    def get_dhcp_packet_type(self):
        return packet.dhcp.DHCP_ACK


class RyuICMPFilter(object):
    def __call__(self, buf):
        pkt = packet.packet.Packet(buf)
        icmp = pkt.get_protocol(packet.icmp.icmp)
        if not icmp:
            return False
...
class RyuICMPPingFilter(RyuICMPFilter):
    ...
    def filter_icmp(self, pkt, icmp):
        if icmp.type != packet.icmp.ICMP_ECHO_REQUEST:
            return False
        result = True
...
class RyuICMPPongFilter(RyuICMPFilter):
    ...
    def filter_icmp(self, pkt, icmp):
        if icmp.type != packet.icmp.ICMP_ECHO_REPLY:
            return False
        ping = self.get_ping()
