...
def run_server(nb_api):
    ...
    data_str = [end-start, cfg.CONF.df_db_test.count,
                (cfg.CONF.df_db_test.count/(end-start))]
...
def run_client(nb_api):
    ...
    def generate_status():
        ...
        # get time when first event was received
        start = events[0][0]
    ...
    def finished():
        ...
        outfile = open(outfile_name, 'w')
        outfile.write('min, max, median, average, mean, std, var, sum, total, count, r/sec'
                      '\n')
        outfile.write(', '.join(data_str))
        outfile.write('\n')
        outfile.close()
