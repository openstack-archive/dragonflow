...
class TestL3App(test_base.DFTestBase):
    ...
    def _create_packet(self, dst_ip, proto, ttl=255):
        ...
        result.serialize()
        LOG.error("SHACHAR - Prepared %d packet to: %s", proto, dst_ip)
        return result.data
    ...
    def _create_pong_packet(self, buf):
        ...
        LOG.error("SHACHAR - Detected ping, from %s sending pong to %s",
                  ip.src, ip.dst)
    ...
    def test_reconnect_of_controller(self):
        cmd = ["ovs-vsctl", "show"]
        config = utils.execute(cmd, run_as_root=True).strip()
        LOG.error("SHACHAR - configuration: %s", config)
