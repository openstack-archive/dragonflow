...
class TestDNATApp(test_base.DFTestBase):
    def _get_external_network_id(self):
        ...
        tapname = 'tap-test-{0}'.format(os.getpid() % 100000)
        for command in (
            ['ip', 'tuntap', 'add', 'dev', tapname, 'mode', 'tap'],
            ['ip', 'link', 'set', 'dev', tapname, 'up'],
            ['ip', 'addr', 'add', '172.24.4.1/24', 'dev', tapname],
            ['ovs-vsctl', 'add-port', 'br-ex', tapname],
        ):
            test_utils.print_command(command, run_as_root=True)

        def cleanup_interface():
            for command in (
                ['ovs-vsctl', 'del-port', 'br-ex', tapname],
                ['ip', 'link', 'set', 'dev', tapname, 'down'],
                ['ip', 'tuntap', 'del', 'dev', tapname, 'mode', 'tap'],
            ):
                test_utils.print_command(command, run_as_root=True)

        self.store('tap if', close_func=cleanup_interface)
    ...
    def setUp(self):
        ...
        try:
            ...
            self.fip.create({'floating_network_id': external_network_id})
            self.assertTrue(self.fip.exists())
            self.fip.update({'port_id': self.int_port.port.port_id})
