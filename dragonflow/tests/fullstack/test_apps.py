...
class SfcTestsCommonBase(test_base.DFTestBase):
    ...
    def _create_ppg(self):
        pp = self._create_pp()
        ppg = self.store(
            objects.PortPairGroupTestObj(self.neutron, self.nb_api))
        ppg.create_from_portpairs([pp])
        return ppg

    def _create_pc(self, fc, hops):
        ppgs = [self._create_ppg() for _ in range(hops)]
        pc = self.store(
    ...
    def _create_port_policies(self, pc):
        res = {}
        for i, ppg in enumerate(pc.port_pair_groups):
            for j, pp in enumerate(ppg.port_pairs):
...
class TestFcApp(SfcTestsCommonBase):
    def _run_test(self, fc_params, chain_len, initial_packet, final_packet):
        ...
        fc.create(fc_params)
        pc = self._create_pc(fc, chain_len)
