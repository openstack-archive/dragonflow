...
class TestSGApp(test_base.DFTestBase):
    def setUp(self):
        ...
        try:
            ...
            port1_ip = self.port1.get_logical_port().get_ip()
            ingress_rule_info = {'ethertype': 'IPv4',
                                 'direction': 'ingress',
                                 'protocol': 'icmp',
                                 'remote_ip_prefix': port1_ip}
            ...
            icmp_request1 = self._create_ping_packet(self.port1, self.port3)
            icmp_request2 = self._create_ping_packet(self.port2, self.port3)

            port_policies = self._create_port_policies(icmp_request1,
                                                       icmp_request2)
    ...
    def _create_port_policies(self, icmp_request1, icmp_request2):
        ignore_action = app_testing_objects.IgnoreAction()
        raise_action = app_testing_objects.RaiseAction("Unexpected packet")
        ...
        rules1 = [
            app_testing_objects.PortPolicyRule(
                # Detect pong, end simulation
                app_testing_objects.RyuICMPPongFilter(icmp_request1),
                actions=[
                    app_testing_objects.DisableRuleAction(),
                    app_testing_objects.StopSimulationAction(),
                ]
        ...
        rules2 = [
            app_testing_objects.PortPolicyRule(
                ...
                app_testing_objects.RyuICMPPongFilter(icmp_request2),
                actions=[
                    raise_action
                ]
            ),
        ...
        rules3 = [
            app_testing_objects.PortPolicyRule(
                ...
                app_testing_objects.RyuICMPPingFilter(icmp_request1),
                actions=[
                    app_testing_objects.SendAction(
                        self.subnet.subnet_id,
                        self.port3.port_id,
            ...
            app_testing_objects.PortPolicyRule(
                ...
                app_testing_objects.RyuICMPPingFilter(icmp_request2),
                actions=[
                    raise_action
                ]
            ),
