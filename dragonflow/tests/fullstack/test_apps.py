...
class TestSNat(test_base.DFTestBase):
    ...
    def setUp(self):
        ...
        # Setup
        """
        sudo ip netns add test-snat
        sudo ip link add type veth
        sudo ip link set veth1 netns test-snat
        sudo ip netns exec test-snat ip link set veth1 up
        sudo ip netns exec test-snat ip addr add 10.0.0.2/24 dev veth1
        sudo ip link set veth0 up
        sudo ip addr add 10.0.0.1/24 dev veth0
        sudo ip netns exec test-snat ip route add default via 10.0.0.1
        """

        ipwrapper = ip_lib.IPWrapper()
        #ipwrapper.netns.add(self.namespace_name)
