...
class TestSGApp(test_base.DFTestBase):
    def setUp(self):
        ...
        try:
            ...
            port1_ip = self.port1.port.get_logical_port().get_ip()
            ...
            self.icmp_request1 = self._create_ping_packet(self.port1, self.port3)
            self.icmp_request2 = self._create_ping_packet(self.port2, self.port3)
                ...
                app_testing_objects.Policy(
                    initial_actions=[
                        app_testing_objects.SendAction(
                            ...
                            self.port1.port_id,
                            self.icmp_request1.data
                        ),
                        app_testing_objects.SendAction(
                            self.subnet.subnet_id,
                            self.port2.port_id,
                            self.icmp_request2.data
                        )
                    ],
                    port_policies=port_policies,
                    unknown_port_action=app_testing_objects.IgnoreAction()
