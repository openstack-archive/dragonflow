...
class TestL2FLows(test_base.DFTestBase):
    ...
    def test_vlan_network_flows(self):
        ...
        if network_vlan_range_list is None \
                or len(network_vlan_range_list) == 0:
            return

        network_vlan_range = network_vlan_range_list[0]
        vlan_params = network_vlan_range.split(':')

        if vlan_params is None or len(vlan_params) == 0:
            return

        physical_network = vlan_params[0]
        vlan_min = vlan_params[1]
    ...
    def _check_vlan_flows(self, flows, metadtata, segmentation_id,
        ...
        ingress_match = 'dl_vlan=' + segmentation_id
        ingress_action = 'set_field:0x' + metadtata + '->metadata,' \
                                                      'pop_vlan,goto_table:' + \
                         str(const.INGRESS_DESTINATION_PORT_LOOKUP_TABLE)
    ...
    def test_flat_network_flows(self):
        ...
        if flat_networks_list is None or len(flat_networks_list) == 0:
            return
