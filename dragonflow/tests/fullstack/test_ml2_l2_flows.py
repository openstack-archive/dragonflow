...
class TestL2FLows(test_base.DFTestBase):
    @staticmethod
    def _get_metadata_id(flows, ip, mac):
        for flow in flows:
    ...
    @staticmethod
    def _check_tunnel_flows(flows, metadtata, segmentation_id,
                            port_key_hex, mac):
        ...
        if l2_lookup_multicast_check is None or l2_lookup_unicast_check is None or ingress_check is None:
            return None

        return True

    @staticmethod
    def _check_vlan_flows(flows, metadtata, segmentation_id,
                          port_key_hex, mac):
        l2_lookup_unicast_match = 'metadata=0x' + metadtata + \
                                  ',dl_dst=' + mac
        l2_lookup_unicast_action = 'goto_table:' + \
    ...
    @staticmethod
    def _check_flat_flows(flows, metadtata,
                          port_key_hex, mac):
        l2_lookup_unicast_match = 'metadata=0x' + metadtata + \
                                  ',dl_dst=' + mac
        l2_lookup_unicast_action = 'goto_table:' + \
    ...
    @staticmethod
    def _get_config_values(section, key):
        readhandle = None
        try:
