...
class TestNeutronAPIandDB(test_base.DFTestBase):
    ...
    def test_dhcp_port_created(self):
        ...
        ports = utils.wait_until_is_and_return(
            lambda: self.nb_api.lport.get_all(),
    ...
    def test_create_delete_router(self):
        ...
        version1 = self.nb_api.lrouter.get(router_id).get_version()
        router.update()
        self.assertTrue(router.exists())
        version2 = self.nb_api.lrouter.get(router_id).get_version()
    ...
    def test_delete_router_interface_port(self):
        ...
        router_l = self.neutron.add_interface_router(router_id,
                                                     body=interface_msg)
        routers = self.nb_api.lrouter.get_all()
        router2 = None
        for r in routers:
    ...
    def test_create_delete_security_group(self):
        ...
        version1 = self.nb_api.security_group.get(sg_id).get_version()
        secgroup.update()
        self.assertTrue(secgroup.exists())
        version2 = self.nb_api.security_group.get(sg_id).get_version()
