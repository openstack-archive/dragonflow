...
class TestNeutronAPIandDB(test_base.DFTestBase):
    ...
    def test_associate_floatingip(self):
        ...
        port = objects.PortTestWrapper(self.neutron, self.nb_api)
        ...
        port_para = {'admin_state_up': True, 'name': 'port1',
                'network_id': private_network_id}
        port_result = self.neutron.create_port(body={'port': port_para})
        port_result = port_result['port']
        lport = self.nb_api.get_logical_port(port_result['id'])
        self.assertIsNotNone(lport)
        self.assertEqual(private_network_id, lport.get_lswitch_id())

        fip_para = {'floating_network_id': external_network_id}
        # create
        fip1 = fip.create(fip_para)
        self.assertTrue(fip.exists())

        # associate with port
        fip1['port_id'] = port_result['id']
        fip.update(fip1['id'], fip1)
        fip2 = self.nb_api.get_floatingip(fip1['id'])
        self.assertTrue(fip1['port_id'] == fip2['port_id'])
    ...
    def test_disassociate_floatingip(self):
        ...
        del fip1['port_id']
        del fip1['fixed_ip_address']
        fip.update(fip1['id'], fip1)
        fip2 = self.nb_api.get_floatingip(fip1['id'])
        self.assertFalse(fip2.get('port_id', False))
