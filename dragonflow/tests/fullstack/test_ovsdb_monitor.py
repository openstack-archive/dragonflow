...
class TestOvsdbMonitor(test_base.DFTestBase):
    ...
    def test_notify_message(self):
        ...
        utils.wait_until_none(
            lambda: self.get_vm_port(mac), timeout=10, sleep=1,
    ...
    def test_reply_message(self):
        ...
        vm2.close()
        utils.wait_until_none(
            lambda: self.get_vm_port(mac1), timeout=10, sleep=1,
            exception=Exception('Port was not deleted')
        )
        utils.wait_until_none(
            lambda: self.get_vm_port(mac2), timeout=10, sleep=1,
            exception=Exception('Port was not deleted')
        )
        network.close()
