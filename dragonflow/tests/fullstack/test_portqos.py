...
class TestPortQos(test_base.DFTestBase):
    def test_port_with_qospolicy(self):
        ...
        qospolicy.create_rule(qos_policy_id,
                              {qos_consts.MAX_KBPS: '1000',
                               qos_consts.MAX_BURST: '100'},
                              qos_consts.RULE_TYPE_BANDWIDTH_LIMIT)
        qospolicy.create_rule(qos_policy_id,
                              {qos_consts.DSCP_MARK: '10'},
                              qos_consts.RULE_TYPE_DSCP_MARKING)
        port_param = {qos_consts.QOS_POLICY_ID: qos_policy_id}
    ...
    def test_network_with_qos(self):
        ...
        net_param = {qos_consts.QOS_POLICY_ID: qos_policy_id}
        network.update(net_param)
        time.sleep(const.DEFAULT_CMD_TIMEOUT)
        nb_net = self.nb_api.get(l2.LogicalSwitch(id=network.network_id))
        self.assertEqual(qos_policy_id, nb_net.qos_policy.id)
