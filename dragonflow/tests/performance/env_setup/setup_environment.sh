...
if [ "$(glance image-list | grep iPerfServer | awk '{print $4}')" == "" ]; then
	...
	wget -O /tmp/iPerfServer.img https://cloud-images.ubuntu.com/trusty/20160426/trusty-server-cloudimg-i386-disk1.img
...
echo -e "==========\nCreating two networks and connects them to the router\n=========="
python create_net.py private1 10.1.0.1/16
python create_net.py private2 192.168.0.1/16
...
# nova boot --image iPerfServer --flavor m1.small --nic net-id=dd3905f8-a0b2-4813-a1fb-1a4e4e17b944 --availability-zone nova:saturn --key-name=ssh-key vmName
for ((i=1;i<=numOfVMs;i++));
do
	echo -e "==========\nCreating iPerf-client$i VM on $computeNode\n=========="
	python create_VM.py iPerf-client$i private1 $computeNode
	echo -e "==========\nCreating iPerf-server$i VM on $controllerNode\n=========="
	python create_VM.py iPerf-server$i private1 $controllerNode
done
sleep 5
for ((i=1;i<=numOfVMs;i++));
do
	echo -e "==========\nCreating iPerf-server$i VM on $controllerNode\n=========="
	python create_VM.py iPerf-server$(($i+1)) private2 $controllerNode
done
sleep 5
for ((i=1;i<=numOfVMs;i++));
do
	echo -e "==========\nCreating iPerf-server$i VM on $computeNode\n=========="
	python create_VM.py iPerf-server$(($i+2)) private1 $computeNode
done
sleep 5
for ((i=1;i<=numOfVMs;i++));
do
	echo -e "==========\nCreating iPerf-server$i VM on $computeNode\n=========="
	python create_VM.py iPerf-server$(($i+3)) private2 $computeNode
done
