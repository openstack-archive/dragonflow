...
bandwidth() {
	...
	for ((i=0; i<numOfRuns; i++)); do
		...
		iperf -c $IP -t 50 | tee -a $2.txt $IP_$2.txt
		sleep 5
	done
}
...
bandwidthLoop() {
	...
	do
		for ((i=0; i<numOfRuns; i++)); do
			echo "$2 test on $IP run #: $i"
			iperf -c $IP -t 50 | tee -a $2.txt $IP.txt
			sleep 5
		done
	done
}
...
udp() {
	...
	do
		for ((i=0; i<numOfRuns; i++)); do
			echo "$2 test on $IP run #: $i"
			iperf -c $IP -u -p 8686 -t 50 | tee -a $2.txt $IP.txt
			sleep 5
		done
	done
}
...
udp30MB() {
	...
	do
		for ((i=0; i<numOfRuns; i++)); do
			...
			iperf -c $IP -u -b 30MB -p 8686 -t 50 | tee -a $2_30MB.txt $IP.txt
			sleep 5
		done
	done
}
...
latency() {
	...
	do
		for ((i=0; i<numOfRuns; i++)); do
			echo "$2 test on $IP run #: $i"
			ping -c 50 -i 0.2 -w 3 $IP | tee -a $2.txt $IP.txt
			sleep 1
		done
	done
}
...
packetsPerSecond() {
	...
	do
		for ((i=0; i<numOfRuns; i++)); do
			echo "$2 test on $IP run #: $i"
			sudo bash -c "timeout 50 hping3 $IP -q -i u20 --icmp|tail -n10 >/dev/null" 2>> $2.txt
			sleep 5
		done
