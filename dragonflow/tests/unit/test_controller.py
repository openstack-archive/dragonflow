...
class TestDfController(test_app_base.DFAppTestBase):
    ...
    def test_register_chassis(self):
        ...
        old_chassis = {"id": 'fake_host',
                       "ip": '192.168.202.110',
                       "tunnel_type": 'nvgre'}
        local_chassis = models.Chassis(chassis)
        old_local_chassis = models.Chassis(old_chassis)
    ...
    def test_tunnel_should_be_added(self):
        ...
        _new_chassis = {"id": 'chassis2',
                        "ip": '192.168.202.205',
                        "tunnel_type": 'vxlan'}
        local_chassis = models.Chassis(_local_chassis)
        diff_tunnel_type = models.Chassis(_diff_type_chassis)
        new_chassis = models.Chassis(_new_chassis)
        self.assertFalse(
                self.controller._tunnel_should_be_created(local_chassis))
        self.assertFalse(
                self.controller._tunnel_should_be_created(diff_tunnel_type))
    ...
    def test_chassis_created(self):
        ...
        _new_chassis = {"id": 'chassis3',
                        "ip": '192.168.202.225',
                        "tunnel_type": 'vxlan'}
        existed_chassis = models.Chassis(_existed_chassis)
        new_chassis = models.Chassis(_new_chassis)
        ip_changed_chassis = models.Chassis(_ip_changed_chassis)
