...
class TestDbStore(tests_base.BaseTestCase):
    ...
    def test_lswitch(self):
        self.db_store.update_lswitch('id1', 'value1', 'topic1')
        self.db_store.update_lswitch('id2', 'value2', 'topic2')
        self.db_store.update_lswitch('id3', 'value3', 'topic2')
        ...
        lswitchs = self.db_store.get_all_lswitches()
        lswitchs_topic2 = self.db_store.get_all_lswitches('topic2')
        ...
        self.db_store.delete_lswitch('id3', 'topic2')
    ...
    def test_port(self):
        ...
        self.db_store.update_lport('id1', port1, False, 'topic1')
        self.db_store.update_lport('id2', port2, False, 'topic2')
        self.db_store.update_lport('id3', port3, False, 'topic2')
        self.db_store.update_lport('id4', port4, True, 'topic2')
        port_keys = self.db_store.get_lport_keys()
        port_keys_topic2 = self.db_store.get_lport_keys('topic2')
        self.assertEqual({'id1', 'id2', 'id3', 'id4'}, set(port_keys))
        self.assertIn('id2', port_keys_topic2)
        self.assertIn('id3', port_keys_topic2)
        ports = self.db_store.get_all_lports()
        ports_topic2 = self.db_store.get_all_lports('topic2')
        self.assertEqual({port1, port2, port3, port4}, set(ports))
        self.assertIn(port2, ports_topic2)
        self.assertIn(port3, ports_topic2)
        self.assertEqual(port1, self.db_store.get_lport('id1'))
        self.assertEqual(port2, self.db_store.get_lport('id2'))
        self.assertEqual(
            port1,
            self.db_store.get_lport('id1', 'topic1'),
        )
        ...
        self.db_store.delete_lport('id4', True, 'topic2')
        self.assertIsNone(
            self.db_store.get_local_port('id4', 'topic2')
        )
        self.assertIsNone(
            self.db_store.get_lport('id4', 'topic2')
        )
        self.assertEqual(
            {port2, port3},
            set(self.db_store.get_ports_by_network_id('net1'))
        )
        self.db_store.delete_lport('id3', False, 'topic2')
        self.assertIsNone(self.db_store.get_lport('id3'))
    ...
    def test_security_group(self):
        ...
        sg2 = 'sg2'
        sg3 = 'sg3'
        self.db_store.update_secgroup('id1', sg1, 'topic1')
        self.db_store.update_secgroup('id2', sg2, 'topic2')
        self.db_store.update_secgroup('id3', sg3, 'topic2')
        self.assertEqual(sg1, self.db_store.get_secgroup('id1'))
        self.assertEqual(sg2, self.db_store.get_secgroup('id2'))
        self.assertEqual(
            sg3,
            self.db_store.get_secgroup('id3', 'topic2')
        )
        sg_keys = self.db_store.get_secgroup_keys()
        sg_keys_topic2 = self.db_store.get_secgroup_keys('topic2')
        sgs = self.db_store.get_all_secgroups()
        sgs_topic2 = self.db_store.get_all_secgroups('topic2')
        ...
        self.db_store.delete_secgroup('id3', 'topic2')
        self.assertIsNone(self.db_store.get_secgroup('id3', 'topic2'))
