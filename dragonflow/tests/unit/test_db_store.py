...
class TestDbStore(tests_base.BaseTestCase):
    ...
    def test_chassis(self):
        chassis1_params = {'id': 'chassis1', 'ip': '1.1.1.1'}
        chassis2_params = {'id': 'chassis2', 'ip': '2.2.2.2'}
        chassis1 = core_models.Chassis(**chassis1_params)
        chassis2 = core_models.Chassis(**chassis2_params)
        self.db_store2.update(chassis1)
        self.db_store2.update(chassis2)
        self.assertEqual(
            chassis1,
            self.db_store2.get_one(core_models.Chassis(**chassis1_params))
        )
        self.assertEqual(
            chassis2,
            self.db_store2.get_one(core_models.Chassis(**chassis2_params))
        )
        self.db_store2.delete(core_models.Chassis(**chassis1_params))
        self.assertIsNone(self.db_store2.get_one(
            core_models.Chassis(**chassis1_params)))

        self.db_store2.delete(core_models.Chassis(**chassis2_params))
        self.assertIsNone(self.db_store2.get_one(
            core_models.Chassis(**chassis2_params)))
