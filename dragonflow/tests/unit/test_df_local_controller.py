...
class DfLocalControllerTestCase(test_app_base.DFAppTestBase):
    ...
    def test_floatingip_updated(self, mock_get_lport, mock_get_fip,
                                mock_notify_asso, mock_notify_dis,
        ...
        local_lport = mock.Mock()
        mock_get_lport.return_value = local_lport
        self.assertIsNone(self.controller.update_floatingip(fip))
        ...
        self.assertIsNone(self.controller.update_floatingip(fip))
        mock_notify_asso.assert_called_once_with(fip)

        old_fip = mock.Mock()
        old_fip.get_lport_id.return_value = 'fake_lport_id1'
        mock_get_fip.return_value = old_fip
        mock_notify_asso.reset_mock()
        self.assertIsNone(self.controller.update_floatingip(fip))
        mock_notify_asso.assert_called_once_with(fip)
        mock_notify_dis.assert_called_once_with(old_fip)
