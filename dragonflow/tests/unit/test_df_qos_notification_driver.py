...
class TestDFQosNotificationDriver(test_mech_driver.DFMechanismDriverTestCase):
    ...
    def test_create_update_network_qos_policy(self):
        ...
        with self.network(arg_list=('qos_policy_id',), **kwargs) as n:
            ...
            called_args = nb_api.create.call_args_list[2][0][0]
    ...
    def test_create_update_port_qos_policy(self):
        ...
        with self.subnet(enable_dhcp=False) as subnet:
            self.driver.nb_api.create.reset_mock()
            with self.port(subnet=subnet,
                           arg_list=('qos_policy_id',),
                           **kwargs) as p:
                port_id = p['port']['id']
                self.driver.nb_api.create.assert_called_once()
                lport = self.driver.nb_api.create.call_args_list[0][0][0]
                self.assertIsInstance(lport, l2.LogicalPort)
                self.assertEqual(qos_obj['id'], lport.qos_policy.id)

                self.driver.nb_api.update.reset_mock()
                data = {'port': {'qos_policy_id': None}}
                req = self.new_update_request('ports', data, port_id)
                req.get_response(self.api)
                self.driver.nb_api.update.assert_called_once()
                lport = self.driver.nb_api.update.call_args_list[0][0][0]
                self.assertIsInstance(lport, l2.LogicalPort)
