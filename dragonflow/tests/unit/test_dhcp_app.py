...
class TestDHCPApp(test_app_base.DFAppTestBase):
    ...
    def test_port_not_updated_ipv6(self):
        dhcp_port = self._create_dhcp_port()
        self.app._lport_created(dhcp_port)
        new_port = copy.deepcopy(dhcp_port)
        new_port.ips = dhcp_port.ips + ["1111::1"]

        self.app._update_port_responders = mock.Mock()
        self.app._lport_updated(dhcp_port, dhcp_port)
        self.app._update_port_responders.assert_not_called()
