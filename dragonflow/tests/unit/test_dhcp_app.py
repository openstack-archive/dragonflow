...
class TestDHCPApp(test_app_base.DFAppTestBase):
    ...
    def test_get_lswitch_by_port(self):
        lport = mock.Mock()
        l_switch_id = 'lswitchId'
        lport.get_lswitch_id.return_value = l_switch_id
        fake_lswitch = copy.deepcopy(test_app_base.fake_logic_switch1)
        self.app.db_store.set_lswitch(l_switch_id, fake_lswitch)
        lswitch = self.app._get_lswitch_by_port(lport)
        lport.get_lswitch_id.assert_called_once_with()
        self.assertEqual(lswitch, fake_lswitch)

    def test_get_dhcp_message_type_opt(self):
        ...
        self.assertEqual(opt_value2, None)

    def test_get_subnet_by_port(self):
        ...
        self.assertEqual(subnet, fake_lport_subnet)
    ...
    def test_remove_logical_switch(self):
        ...
        self.assertFalse(network_id in self.app.switch_dhcp_ip_map)
