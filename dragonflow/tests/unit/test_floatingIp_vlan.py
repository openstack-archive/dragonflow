...
class TestFloatingIpwithVlan(tests_base.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.DNATApp = DNATApp(api=mock.Mock(),
                               vswitch_api='test_vswitch_api',
                               nb_api='test_nb_api')
    ...
    def test_del_network_flows_for_ingress_vlan(self):
        result = self.DNATApp._del_network_flows_for_ingress_vlan('2')
        self.assertEqual(result, None)
    ...
    def test_del_network_flows_for_ingress_flat(self):
        ...
        result = self.DNATApp._del_network_flows_for_ingress_flat()
        self.assertEqual(result, None)

    def test_install_network_flows_for_ingress_tunnel(self):

        result = self.DNATApp._install_network_flows_for_ingress_tunnel(
            '2', '2')
        self.assertEqual(result, None)

    def test_del_network_flows_for_ingress_tunnel(self):

        result = self.DNATApp._del_network_flows_for_ingress_tunnel('2')
        self.assertEqual(result, None)
