...
class TestDFMechDriver(base.BaseTestCase):
    ...
    def test_delete_security_group_rule(self):
        ...
        with mock.patch.object(manager.NeutronManager, 'get_plugin',
            ...
            kwargs = self._get_security_group_rule_kwargs(
                rule, sgr_id, context)
            ...
            # TODO(xiaohhui): The following fake can be removed once
            # https://review.openstack.org/#/c/367728/ has been merged
            # in neutron.
            fake_sg_rule = mock.Mock()
            fake_sg_rule.get_id = mock.Mock(return_value=sgr_id)
            fake_sg = mock.Mock()
            fake_sg.get_id = mock.Mock(return_value=sg_id)
            fake_sg.get_rules = mock.Mock(return_value=[fake_sg_rule])
            self.driver.nb_api.get_security_groups = mock.Mock(
...
class TestDFMechDriverRevision(test_plugin.Ml2PluginV2TestCase):
    ...
    def setUp(self):
        ...
        mech_driver = mm.mech_drivers['df'].obj
        self.nb_api = mech_driver.nb_api = mock.MagicMock()
    ...
    def _test_create_security_group_revision(self):
        ...
        self.nb_api.create_security_group.assert_called_with(
            ...
            name=sg['name'], rules=mock.ANY, version=sg['revision_number'])
        return sg
    ...
    def test_create_delete_sg_rule_revision(self):
        ...
        self.nb_api.add_security_group_rules.assert_called_with(
            sg['id'], sg['tenant_id'],
            sg_rules=mock.ANY, sg_version=new_sg['revision_number'])

        # TODO(xiaohhui): The following fake can be removed once
        # https://review.openstack.org/#/c/367728/ has been merged
        # in neutron.
        fake_sg_rule = mock.Mock()
        fake_sg_rule.get_id = mock.Mock(return_value=rule['id'])
        fake_sg = mock.Mock()
        fake_sg.get_id = mock.Mock(return_value=sg['id'])
        fake_sg.get_rules = mock.Mock(return_value=[fake_sg_rule])
        self.nb_api.get_security_groups = mock.Mock(return_value=[fake_sg])
    ...
    def test_create_update_delete_subnet_network_revision(self):
        ...
        self.assertGreater(new_network['revision_number'],
                           network['revision_number'])
        self.nb_api.delete_subnet(subnet_id, network['id'],
                                  subnet['tenant_id'],
                                  nw_version=new_network['revision_number'])
