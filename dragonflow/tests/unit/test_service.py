...
class TestServiceHealth(tests_base.BaseTestCase):
    ...
    def test_init(self):
        with mock.patch.object(report_status, 'run_status_reporter') as rsr:
            service.Service('test_binary')
    ...
    @mock.patch(
        'dragonflow.db.neutron.lockedobjects_db._test_and_create_object'
    )
    @mock.patch('dragonflow.db.neutron.lockedobjects_db._acquire_lock')
    @mock.patch('dragonflow.db.neutron.lockedobjects_db._release_lock')
    def test_update_last_seen(self, _rl, _al, _taco):
        s = service_model.Service(chassis='test_host1', binary='test_binary')
        ...
        updated_service = self.nb_api.update.call_args_list[0][0][0]
