...
class OvsInfo():
    ...
    def _ovs_bitify_tables(self, flows):
        ...
        for flow in flows:
            ...
            flow['actions'] = re.sub(r'set_field:0x([0-9a-fA-F]+)[-]>metadata', r'NETWORK=0x\1', flow['actions'])
            flow['match'] = re.sub(r'metadata=0x([0-9a-fA-F]+)', r'NETWORK=0x\1', flow['match'])
            flow['actions'] = re.sub(r'set_field:0x([0-9a-fA-F]+)->reg6', r'STAG=0x\1', flow['actions'])
            flow['match'] = re.sub(r'reg6=0x([0-9a-fA-F]+)', r'STAG=0x\1', flow['match'])
            flow['actions'] = re.sub(r'set_field:0x([0-9a-fA-F]+)->reg7', r'DTAG=0x\1', flow['actions'])
            flow['match'] = re.sub(r'reg7=0x([0-9a-fA-F]+)', r'DTAG=0x\1', flow['match'])
            flow['match'] = re.sub('tun_id=', 'VTAG=', flow['match'])
            result.append(flow)
        return result


    def ovs_list_ports(self):
        ports = self._get_ovs_local_ports()
        ports = self._parse_ovs_local_ports(ports)
        return ports

    def print_ovs_flow_row(self, row):
    ...
    def print_data(self):
        ...
        ovs_ports = self.ovs_list_ports()
        for ovs_port in ovs_ports:
            id = ovs_port['name'][3:]
            if id in neutron_ports:
                info = neutron_ports[id]
                netinfo = list()
                ...
                print("in_port_"+ovs_port['id'], ovs_port['name'], ovs_port['addr'], info['name'], info['device_owner'], info['mac_address'], netinfo)
                print(info)
            else:
                print("in_port_"+ovs_port['id'], ovs_port['name'], ovs_port['addr'], '-----------------')
        flows = self.list_flows()
        print("=" * 130)
        print("TABLE:     | PRI | PATTERN                                            | ACTION                                   | More")
        print("=" * 130)
        for flow in flows:
            self.print_ovs_flow_row(flow)
