...
function zuul_install_project {
    PROJ=$1
    # Clone the projects only if running under zuul v2
    ...
    if [ -n "$LOG_PATH" ]; then
        ...
        # We assume the project exists. This is done by zuul when the project
        # exists in the required-projects list
